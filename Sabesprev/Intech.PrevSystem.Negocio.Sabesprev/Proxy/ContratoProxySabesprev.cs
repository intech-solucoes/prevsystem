...
namespace Intech.PrevSystem.Negocio.Sabesprev.Proxy
{
    public class ContratoProxySabesprev : ContratoProxy
    {
        public List<ContratoEntidade> BuscarPorFundacaoEmpresaPlanoInscricaoSituacao(string CD_FUNDACAO, string CD_EMPRESA, string CD_PLANO, string NUM_INSCRICAO, string CD_SITUACAO)
        {
            ...
            var listaRetorno = new List<ContratoEntidade>();

            listaContratos.ForEach(contrato =>
            {
                // Verifica o ultimo plano do contrato
                var planosContrato = new PlanosContratoProxy().BuscarPorFundacaoAnoNumContrato(CD_FUNDACAO, contrato.ANO_CONTRATO, contrato.NUM_CONTRATO);
                if (planosContrato.First().CD_PLANO == CD_PLANO)
                {
        ...
        public string Contratar(FuncionarioEntidade funcionario, ContratoDisponivel contrato, ConcessaoEntidade concessao, SaldoDevedorEntidade saldoDevedor, string num_gr_fam, int sqRecebedor)
        {
            using (var transaction = new TransactionScope())
            {
                ...
                var recebedorBeneficio = new RecebedorBeneficioProxy().BuscarPorFundacaoEmpresaInscricaoSeqRecebedor(funcionario.CD_FUNDACAO, funcionario.CD_EMPRESA, funcionario.NUM_INSCRICAO, sqRecebedor.ToString());
                ...
                new ContratoWebProxy().InserirContratoWeb(
                    ...
                    NUM_SEQ_GR_FAMIL: Convert.ToDecimal(num_gr_fam),													//NUM_SEQ_GR_FAMIL
                    ...
                    VL_REMUNERACAO: concessao.ValorUltimoSalario,		//VL_REMUNERACAO
