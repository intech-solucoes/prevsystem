...
namespace Intech.PrevSystem.Dados.DAO
{
	...
	{
		public UsuarioDAO (IDbTransaction tx = null) : base(tx) { }
		...
		public virtual void FSF_InsertNaoParticipante(string NOM_LOGIN, string PWD_USUARIO, string IND_BLOQUEADO, decimal NUM_TENTATIVA, string DES_LOTACAO, string IND_ADMINISTRADOR, string IND_ATIVO, string NOM_USUARIO_CRIACAO, DateTime? DTA_CRIACAO, string NOM_USUARIO_ATUALIZACAO, DateTime? DTA_ATUALIZACAO, string CD_EMPRESA, string IND_PRIMEIRO_ACESSO, decimal? SEQ_RECEBEDOR, string IND_NAO_PARTICIPANTE)
		{
			try
			{
				if (AppSettings.IS_SQL_SERVER_PROVIDER)
					Conexao.Execute("INSERT INTO WEB_USUARIO (      NOM_LOGIN,      PWD_USUARIO,      IND_BLOQUEADO,      NUM_TENTATIVA,      DES_LOTACAO,      IND_ADMINISTRADOR,      IND_ATIVO,      NOM_USUARIO_CRIACAO,      DTA_CRIACAO,      NOM_USUARIO_ATUALIZACAO,      DTA_ATUALIZACAO,      CD_EMPRESA,      SEQ_RECEBEDOR,      IND_PRIMEIRO_ACESSO,  	IND_NAO_PARTICIPANTE  ) VALUES (      @NOM_LOGIN,      @PWD_USUARIO,      @IND_BLOQUEADO,      @NUM_TENTATIVA,      @DES_LOTACAO,      @IND_ADMINISTRADOR,      @IND_ATIVO,      @NOM_USUARIO_CRIACAO,      @DTA_CRIACAO,      @NOM_USUARIO_ATUALIZACAO,      @DTA_ATUALIZACAO,      @CD_EMPRESA,      @SEQ_RECEBEDOR,      @IND_PRIMEIRO_ACESSO,  	@IND_NAO_PARTICIPANTE  )", new { NOM_LOGIN, PWD_USUARIO, IND_BLOQUEADO, NUM_TENTATIVA, DES_LOTACAO, IND_ADMINISTRADOR, IND_ATIVO, NOM_USUARIO_CRIACAO, DTA_CRIACAO, NOM_USUARIO_ATUALIZACAO, DTA_ATUALIZACAO, CD_EMPRESA, IND_PRIMEIRO_ACESSO, SEQ_RECEBEDOR, IND_NAO_PARTICIPANTE });
				else if (AppSettings.IS_ORACLE_PROVIDER)
					Conexao.Execute("", new { NOM_LOGIN, PWD_USUARIO, IND_BLOQUEADO, NUM_TENTATIVA, DES_LOTACAO, IND_ADMINISTRADOR, IND_ATIVO, NOM_USUARIO_CRIACAO, DTA_CRIACAO, NOM_USUARIO_ATUALIZACAO, DTA_ATUALIZACAO, CD_EMPRESA, IND_PRIMEIRO_ACESSO, SEQ_RECEBEDOR, IND_NAO_PARTICIPANTE });
				else
					throw new Exception("Provider não suportado!");
			}
			finally
