...
SELECT FF.CD_FUNDACAO,
       ...
       ROUND(ISNULL((SELECT SUM(ISNULL(CASE WHEN FF2.CD_OPERACAO = 'D' THEN FF2.CONTRIB_PARTICIPANTE * -1 ELSE FF2.CONTRIB_PARTICIPANTE END,0)) AS CONTRIB_PARTICIPANTE 
         ...
         WHERE TC2.COD_AGRUPADOR_WEB = 'BAS'
           ...
           AND FF2.MES_REF       = FF.MES_REF),0),2) AS CONTRIB_PARTICIPANTE,
       ...
       ISNULL((SELECT SUM(ISNULL(CASE WHEN FF2.CD_OPERACAO = 'D' THEN FF2.QTD_COTA_RP_PARTICIPANTE * -1 ELSE FF2.QTD_COTA_RP_PARTICIPANTE END,0)) AS QTD_COTA_RP_PARTICIPANTE
        ...
        /* contribuição básica patrocinadora em reais */
          ROUND(ISNULL((SELECT SUM(ISNULL(CASE WHEN FF2.CD_OPERACAO = 'D' THEN FF2.CONTRIB_EMPRESA * -1 ELSE FF2.CONTRIB_EMPRESA END,0)) AS  CONTRIB_EMPRESA
          FROM CC_FICHA_FINANCEIRA FF2 
               INNER JOIN TB_TIPO_CONTRIBUICAO TC2 ON TC2.CD_TIPO_CONTRIBUICAO = FF2.CD_TIPO_CONTRIBUICAO
         WHERE TC2.COD_AGRUPADOR_WEB = 'BAS'
           AND FF2.CD_FUNDACAO   = FF.CD_FUNDACAO
           ...
           AND FF2.CD_PLANO      = FF.CD_PLANO
           AND FF2.ANO_REF       = FF.ANO_REF
           AND FF2.MES_REF       = FF.MES_REF),0),2) AS CONTRIB_EMPRESA,     
       /* contribuição básica patrocinadora em cotas */
        ...
        ROUND(ISNULL((SELECT SUM(ISNULL(CASE WHEN FF2.CD_OPERACAO = 'D' THEN FF2.CONTRIB_PARTICIPANTE * -1 ELSE FF2.CONTRIB_PARTICIPANTE END,0)) AS RISCO_PART 
          FROM CC_FICHA_FINANCEIRA FF2 
               INNER JOIN TB_TIPO_CONTRIBUICAO TC2 ON TC2.CD_TIPO_CONTRIBUICAO = FF2.CD_TIPO_CONTRIBUICAO
         WHERE TC2.COD_AGRUPADOR_WEB = 'RIS'
           ...
           AND FF2.CD_PLANO      = FF.CD_PLANO
           AND FF2.ANO_REF       = FF.ANO_REF
           AND FF2.MES_REF       = FF.MES_REF),0),2) AS RISCO_PART,
        /* contribuição risco patrocinadora */
          ROUND(ISNULL((SELECT SUM(ISNULL(CASE WHEN FF2.CD_OPERACAO = 'D' THEN FF2.CONTRIB_EMPRESA * -1 ELSE FF2.CONTRIB_EMPRESA END,0)) AS  RISCO_PATROC
          FROM CC_FICHA_FINANCEIRA FF2 
               INNER JOIN TB_TIPO_CONTRIBUICAO TC2 ON TC2.CD_TIPO_CONTRIBUICAO = FF2.CD_TIPO_CONTRIBUICAO
         WHERE TC2.COD_AGRUPADOR_WEB = 'RIS'
           AND FF2.CD_FUNDACAO   = FF.CD_FUNDACAO
           ...
           AND FF2.CD_PLANO      = FF.CD_PLANO
           AND FF2.ANO_REF       = FF.ANO_REF
           AND FF2.MES_REF       = FF.MES_REF),0),2) AS RISCO_PATROC,
        /* contribuição administrativa participante */
        ROUND(ISNULL((SELECT SUM(ISNULL(CASE WHEN FF2.CD_OPERACAO = 'D' THEN FF2.CONTRIB_PARTICIPANTE * -1 ELSE FF2.CONTRIB_PARTICIPANTE END,0)) AS ADM_PART 
          FROM CC_FICHA_FINANCEIRA FF2 
               INNER JOIN TB_TIPO_CONTRIBUICAO TC2 ON TC2.CD_TIPO_CONTRIBUICAO = FF2.CD_TIPO_CONTRIBUICAO
         WHERE TC2.COD_AGRUPADOR_WEB = 'ADM'
           ...
           AND FF2.CD_PLANO      = FF.CD_PLANO
           AND FF2.ANO_REF       = FF.ANO_REF
           AND FF2.MES_REF       = FF.MES_REF),0),2) AS ADM_PART,
        /* contribuição administrativa patrocinadora */
        ROUND(ISNULL((SELECT SUM(ISNULL(CASE WHEN FF2.CD_OPERACAO = 'D' THEN FF2.CONTRIB_EMPRESA * -1 ELSE FF2.CONTRIB_EMPRESA END,0)) AS  ADM_PATROC
          FROM CC_FICHA_FINANCEIRA FF2 
               INNER JOIN TB_TIPO_CONTRIBUICAO TC2 ON TC2.CD_TIPO_CONTRIBUICAO = FF2.CD_TIPO_CONTRIBUICAO
         WHERE TC2.COD_AGRUPADOR_WEB = 'ADM'
           AND FF2.CD_FUNDACAO   = FF.CD_FUNDACAO
           ...
           AND FF2.CD_PLANO      = FF.CD_PLANO
           AND FF2.ANO_REF       = FF.ANO_REF
           AND FF2.MES_REF       = FF.MES_REF),0),2) AS ADM_PATROC,
        /* contribuição voluntária participante em reais */
        ROUND(ISNULL((SELECT SUM(ISNULL(CASE WHEN FF2.CD_OPERACAO = 'D' THEN FF2.CONTRIB_PARTICIPANTE * -1 ELSE FF2.CONTRIB_PARTICIPANTE END,0)) AS VOLUNTARIA_PART 
          FROM CC_FICHA_FINANCEIRA FF2 
               INNER JOIN TB_TIPO_CONTRIBUICAO TC2 ON TC2.CD_TIPO_CONTRIBUICAO = FF2.CD_TIPO_CONTRIBUICAO
         WHERE TC2.COD_AGRUPADOR_WEB = 'VOL'
           ...
           AND FF2.CD_PLANO      = FF.CD_PLANO
           AND FF2.ANO_REF       = FF.ANO_REF
           AND FF2.MES_REF       = FF.MES_REF),0),2) AS VOLUNTARIA_PART,
        ...
        ROUND(ISNULL((SELECT SUM(ISNULL(CASE WHEN FF2.CD_OPERACAO = 'D' THEN FF2.CONTRIB_PARTICIPANTE * -1 ELSE FF2.CONTRIB_PARTICIPANTE END,0)) AS PORTABILIDADE 
          FROM CC_FICHA_FINANCEIRA FF2 
               INNER JOIN TB_TIPO_CONTRIBUICAO TC2 ON TC2.CD_TIPO_CONTRIBUICAO = FF2.CD_TIPO_CONTRIBUICAO
         WHERE TC2.COD_AGRUPADOR_WEB = 'POR'
           ...
           AND FF2.CD_PLANO      = FF.CD_PLANO
           AND FF2.ANO_REF       = FF.ANO_REF
           AND FF2.MES_REF       = FF.MES_REF),0),2) AS PORTABILIDADE,       
        ...
        ROUND(ISNULL((SELECT SUM(ISNULL(CASE WHEN FF2.CD_OPERACAO = 'D' THEN FF2.CONTRIB_PARTICIPANTE * -1 ELSE FF2.CONTRIB_PARTICIPANTE END,0) + 
         ...
         WHERE TC2.COD_AGRUPADOR_WEB IN ('BAS', 'VOL', 'POR')
           ...
           AND FF2.CD_PLANO      = FF.CD_PLANO
           AND FF2.ANO_REF       = FF.ANO_REF
           AND FF2.MES_REF       = FF.MES_REF),0),2) AS TOTAL_CONTRIB,
       ...
       ROUND((SELECT SUM(ISNULL(CASE WHEN FF2.CD_OPERACAO = 'D' THEN FF2.QTD_COTA_RP_PARTICIPANTE * -1 ELSE FF2.QTD_COTA_RP_PARTICIPANTE END,0) + 
            ISNULL(CASE WHEN FF2.CD_OPERACAO = 'D' THEN FF2.QTD_COTA_RP_EMPRESA * -1 ELSE FF2.QTD_COTA_RP_EMPRESA  END,0)) 
          FROM CC_FICHA_FINANCEIRA FF2 
               INNER JOIN TB_TIPO_CONTRIBUICAO TC2 ON TC2.CD_TIPO_CONTRIBUICAO = FF2.CD_TIPO_CONTRIBUICAO
         WHERE TC2.COD_AGRUPADOR_WEB IN ('BAS', 'VOL', 'POR')
           ...
           AND FF2.NUM_INSCRICAO = FF.NUM_INSCRICAO
           AND FF2.CD_PLANO      = FF.CD_PLANO
           AND FF2.ANO_REF * 13 + FF2.MES_REF <= FF.ANO_REF * 13 + FF.MES_REF) * MAX(IV.VALOR_IND),2) AS SALDO_REAIS,           
       /* Saldo Inicial em Reais */
       ROUND(ISNULL((SELECT SUM(ISNULL(CASE WHEN FF2.CD_OPERACAO = 'D' THEN FF2.QTD_COTA_RP_PARTICIPANTE * -1 ELSE FF2.QTD_COTA_RP_PARTICIPANTE END,0) + 
            ISNULL(CASE WHEN FF2.CD_OPERACAO = 'D' THEN FF2.QTD_COTA_RP_EMPRESA * -1 ELSE FF2.QTD_COTA_RP_EMPRESA  END,0)) 
          FROM CC_FICHA_FINANCEIRA FF2 
               INNER JOIN TB_TIPO_CONTRIBUICAO TC2 ON TC2.CD_TIPO_CONTRIBUICAO = FF2.CD_TIPO_CONTRIBUICAO
         WHERE TC2.COD_AGRUPADOR_WEB IN ('BAS', 'VOL', 'POR')
           ...
           AND FF2.NUM_INSCRICAO = FF.NUM_INSCRICAO
           AND FF2.CD_PLANO      = FF.CD_PLANO
           AND FF2.ANO_REF * 13 + FF2.MES_REF < FF.ANO_REF * 13 + FF.MES_REF) * MAX(IVA.VALOR_IND),0),2) AS SALDO_INICIAL_REAIS, 
       ...
       ROUND(((SELECT SUM(ISNULL(CASE WHEN FF2.CD_OPERACAO = 'D' THEN FF2.QTD_COTA_RP_PARTICIPANTE * -1 ELSE FF2.QTD_COTA_RP_PARTICIPANTE END,0) + 
        ...
        (ISNULL((SELECT SUM(ISNULL(CASE WHEN FF2.CD_OPERACAO = 'D' THEN FF2.CONTRIB_PARTICIPANTE * -1 ELSE FF2.CONTRIB_PARTICIPANTE END,0) + 
         ...
         WHERE TC2.COD_AGRUPADOR_WEB IN ('BAS', 'VOL', 'POR')
           ...
           AND FF2.CD_PLANO      = FF.CD_PLANO
           AND FF2.ANO_REF       = FF.ANO_REF
           AND FF2.MES_REF       = FF.MES_REF),0)),2) AS RENDIMENTO_MES
...
WHERE FF.CD_FUNDACAO   = @CD_FUNDACAO
  ...
  AND IVA.DT_IND = (SELECT MAX(IV3.DT_IND)
                    ...
                    WHERE IV3.COD_IND = IVA.COD_IND
                      AND CAST(MONTH(IV3.DT_IND) AS INT) = CAST(FF.MES_REF AS INT)
                      AND CAST(YEAR(IV3.DT_IND) AS INT)  = CAST(FF.ANO_REF AS INT))
