...
{   
    public abstract class FichaFinanceiraAssistidoDAO : BaseDAO<FichaFinanceiraAssistidoEntidade>
    {
		...
		public virtual IEnumerable<FichaFinanceiraAssistidoEntidade> BuscarDatasPorProcesso(int SQ_PROCESSO)
		{
			try
			{
				if(AppSettings.IS_SQL_SERVER_PROVIDER)
		...
		public virtual IEnumerable<FichaFinanceiraAssistidoEntidade> BuscarPorFundacaoEmpresaMatriculaPlanoReferenciaTipoFolhaIncidLiquidoN(string CD_FUNDACAO, string CD_EMPRESA, string NUM_MATRICULA, string CD_PLANO, DateTime DT_REFERENCIA, string CD_TIPO_FOLHA)
		{
			try
			{
				if(AppSettings.IS_SQL_SERVER_PROVIDER)
					return Conexao.Query<FichaFinanceiraAssistidoEntidade>("SELECT GB_FICHA_FINANC_ASSISTIDO.*,         GB_RUBRICAS_PREVIDENCIAL.DS_RUBRICA,         GB_RUBRICAS_PREVIDENCIAL.RUBRICA_PROV_DESC,         GB_RUBRICAS_PREVIDENCIAL.ID_RUB_SUPLEMENTACAO,         GB_ESPECIE_BENEFICIO.DS_ESPECIE,   	   TB_TIPO_FOLHA.DS_TIPO_FOLHA  FROM GB_FICHA_FINANC_ASSISTIDO  INNER JOIN GB_RECEBEDOR_BENEFICIO ON GB_FICHA_FINANC_ASSISTIDO.SEQ_RECEBEDOR = GB_RECEBEDOR_BENEFICIO.SEQ_RECEBEDOR  INNER JOIN GB_RUBRICAS_PREVIDENCIAL ON GB_RUBRICAS_PREVIDENCIAL.CD_RUBRICA = GB_FICHA_FINANC_ASSISTIDO.CD_RUBRICA  INNER JOIN GB_ESPECIE_BENEFICIO ON GB_ESPECIE_BENEFICIO.CD_ESPECIE = GB_FICHA_FINANC_ASSISTIDO.CD_ESPECIE   INNER JOIN TB_TIPO_FOLHA ON TB_TIPO_FOLHA.CD_TIPO_FOLHA = GB_FICHA_FINANC_ASSISTIDO.CD_TIPO_FOLHA  WHERE GB_FICHA_FINANC_ASSISTIDO.CD_FUNDACAO = @CD_FUNDACAO    AND GB_RECEBEDOR_BENEFICIO.CD_FUNDACAO = @CD_FUNDACAO    AND GB_FICHA_FINANC_ASSISTIDO.CD_EMPRESA = @CD_EMPRESA    AND GB_RECEBEDOR_BENEFICIO.CD_EMPRESA = @CD_EMPRESA    AND GB_FICHA_FINANC_ASSISTIDO.CD_PLANO = @CD_PLANO    AND GB_RECEBEDOR_BENEFICIO.NUM_MATRICULA = @NUM_MATRICULA    AND GB_RUBRICAS_PREVIDENCIAL.INCID_LIQUIDO = 'N'    AND GB_FICHA_FINANC_ASSISTIDO.DT_REFERENCIA = @DT_REFERENCIA    AND GB_FICHA_FINANC_ASSISTIDO.CD_TIPO_FOLHA = @CD_TIPO_FOLHA    AND GB_FICHA_FINANC_ASSISTIDO.CD_RUBRICA IN (SELECT CD_RUBRICA FROM GB_BASE_OUTROS)", new { CD_FUNDACAO, CD_EMPRESA, NUM_MATRICULA, CD_PLANO, DT_REFERENCIA, CD_TIPO_FOLHA });
				else if(AppSettings.IS_ORACLE_PROVIDER)
					return Conexao.Query<FichaFinanceiraAssistidoEntidade>("SELECT GB_FICHA_FINANC_ASSISTIDO.*, GB_RUBRICAS_PREVIDENCIAL.DS_RUBRICA, GB_RUBRICAS_PREVIDENCIAL.RUBRICA_PROV_DESC, GB_RUBRICAS_PREVIDENCIAL.ID_RUB_SUPLEMENTACAO, GB_ESPECIE_BENEFICIO.DS_ESPECIE, TB_TIPO_FOLHA.DS_TIPO_FOLHA FROM GB_FICHA_FINANC_ASSISTIDO INNER  JOIN GB_RECEBEDOR_BENEFICIO  ON GB_FICHA_FINANC_ASSISTIDO.SEQ_RECEBEDOR=GB_RECEBEDOR_BENEFICIO.SEQ_RECEBEDOR INNER  JOIN GB_RUBRICAS_PREVIDENCIAL  ON GB_RUBRICAS_PREVIDENCIAL.CD_RUBRICA=GB_FICHA_FINANC_ASSISTIDO.CD_RUBRICA INNER  JOIN GB_ESPECIE_BENEFICIO  ON GB_ESPECIE_BENEFICIO.CD_ESPECIE=GB_FICHA_FINANC_ASSISTIDO.CD_ESPECIE INNER  JOIN TB_TIPO_FOLHA  ON TB_TIPO_FOLHA.CD_TIPO_FOLHA=GB_FICHA_FINANC_ASSISTIDO.CD_TIPO_FOLHA WHERE GB_FICHA_FINANC_ASSISTIDO.CD_FUNDACAO=:CD_FUNDACAO AND GB_RECEBEDOR_BENEFICIO.CD_FUNDACAO=:CD_FUNDACAO AND GB_FICHA_FINANC_ASSISTIDO.CD_EMPRESA=:CD_EMPRESA AND GB_RECEBEDOR_BENEFICIO.CD_EMPRESA=:CD_EMPRESA AND GB_FICHA_FINANC_ASSISTIDO.CD_PLANO=:CD_PLANO AND GB_RECEBEDOR_BENEFICIO.NUM_MATRICULA=:NUM_MATRICULA AND GB_RUBRICAS_PREVIDENCIAL.INCID_LIQUIDO='N' AND GB_FICHA_FINANC_ASSISTIDO.DT_REFERENCIA=:DT_REFERENCIA AND GB_FICHA_FINANC_ASSISTIDO.CD_TIPO_FOLHA=:CD_TIPO_FOLHA AND GB_FICHA_FINANC_ASSISTIDO.CD_RUBRICA IN (SELECT CD_RUBRICA FROM GB_BASE_OUTROS)", new { CD_FUNDACAO, CD_EMPRESA, NUM_MATRICULA, CD_PLANO, DT_REFERENCIA, CD_TIPO_FOLHA });
				else
					throw new Exception("Provider não suportado!");
		...
		public virtual IEnumerable<FichaFinanceiraAssistidoEntidade> BuscarPorFundacaoEmpresaMatriculaPlanoReferenciaTipoFolhaRecebedorIncidLiquidoN(string CD_FUNDACAO, string CD_EMPRESA, string NUM_MATRICULA, int SEQ_RECEBEDOR, string CD_PLANO, DateTime DT_REFERENCIA, string CD_TIPO_FOLHA)
		{
			try
			{
				if(AppSettings.IS_SQL_SERVER_PROVIDER)
		...
		public virtual IEnumerable<FichaFinanceiraAssistidoEntidade> BuscarRelatorioContracheque(int SQ_PROCESSO, DateTime DT_REFERENCIA)
		{
			try
			{
				if(AppSettings.IS_SQL_SERVER_PROVIDER)
					return Conexao.Query<FichaFinanceiraAssistidoEntidade>("SELECT DISTINCT CR.DT_REFERENCIA,      FF.SQ_PROCESSO  FROM fi_ficha_financ_assistido FF       INNER JOIN fi_rubrica_folha_pagamento RU ON RU.SQ_RUBRICA = FF.SQ_RUBRICA       INNER JOIN fi_cronograma_credito CR ON CR.SQ_CRONOGRAMA = FF.SQ_CRONOGRAMA  WHERE FF.SQ_PROCESSO = @SQ_PROCESSO    AND RU.IR_LANCAMENTO IN ('P', 'D')    AND RU.EE_INCIDE_LIQUIDO = 'S'  ORDER BY CR.DT_REFERENCIA DESC", new { SQ_PROCESSO, DT_REFERENCIA });
				else if(AppSettings.IS_ORACLE_PROVIDER)
					return Conexao.Query<FichaFinanceiraAssistidoEntidade>("SELECT DISTINCT CR.DT_REFERENCIA, FF.SQ_PROCESSO FROM FI_FICHA_FINANC_ASSISTIDO  FF  INNER  JOIN FI_RUBRICA_FOLHA_PAGAMENTO   RU  ON RU.SQ_RUBRICA=FF.SQ_RUBRICA INNER  JOIN FI_CRONOGRAMA_CREDITO   CR  ON CR.SQ_CRONOGRAMA=FF.SQ_CRONOGRAMA WHERE FF.SQ_PROCESSO=:SQ_PROCESSO AND RU.IR_LANCAMENTO IN ('P', 'D') AND RU.EE_INCIDE_LIQUIDO='S' ORDER BY CR.DT_REFERENCIA DESC", new { SQ_PROCESSO, DT_REFERENCIA });
				else
					throw new Exception("Provider não suportado!");
		...
		public virtual IEnumerable<FichaFinanceiraAssistidoEntidade> BuscarResumoIncidLiquidoS(string CD_FUNDACAO, string NUM_INSCRICAO)
		{
			try
			{
				if(AppSettings.IS_SQL_SERVER_PROVIDER)
		...
		public virtual IEnumerable<FichaFinanceiraAssistidoEntidade> BuscarResumoIncidLiquidoSUltimos5Anos(string CD_FUNDACAO, string NUM_INSCRICAO)
		{
			try
			{
				if(AppSettings.IS_SQL_SERVER_PROVIDER)
		...
		public virtual IEnumerable<FichaFinanceiraAssistidoEntidade> BuscarResumoPorRecebedorIncidLiquidoS(string CD_FUNDACAO, string NUM_INSCRICAO, int SEQ_RECEBEDOR)
		{
			try
			{
				if(AppSettings.IS_SQL_SERVER_PROVIDER)
		...
		public virtual IEnumerable<FichaFinanceiraAssistidoEntidade> BuscarResumoPorRecebedorIncidLiquidoSUltimo5Anos(string CD_FUNDACAO, string NUM_INSCRICAO, int SEQ_RECEBEDOR)
		{
			try
			{
				if(AppSettings.IS_SQL_SERVER_PROVIDER)
		...
		public virtual IEnumerable<FichaFinanceiraAssistidoEntidade> BuscarRubricasPorProcessoReferencia(int SQ_PROCESSO, DateTime DT_REFERENCIA)
		{
			try
			{
				if(AppSettings.IS_SQL_SERVER_PROVIDER)
