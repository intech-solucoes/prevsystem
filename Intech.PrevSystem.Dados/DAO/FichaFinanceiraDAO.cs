...
namespace Intech.PrevSystem.Dados.DAO
{
	public abstract class FichaFinanceiraDAO : BaseDAO<FichaFinanceiraEntidade>
	{
		...
		public virtual List<FichaFinanceiraEntidade> BuscarContribuicaoPorFundacaoInscricaoPlanoTipoContribuicaoFixaPreves(string CD_FUNDACAO, string NUM_INSCRICAO, string CD_PLANO)
		{
			try
			{
				if (AppSettings.IS_SQL_SERVER_PROVIDER)
					return Conexao.Query<FichaFinanceiraEntidade>("SELECT TC.DS_TIPO_CONTRIBUICAO, 	   FF.*   FROM CC_FICHA_FINANCEIRA FF   	INNER JOIN TB_TIPO_CONTRIBUICAO TC ON TC.CD_TIPO_CONTRIBUICAO = FF.CD_TIPO_CONTRIBUICAO WHERE FF.CD_FUNDACAO = @CD_FUNDACAO       AND FF.NUM_INSCRICAO = @NUM_INSCRICAO       AND FF.CD_PLANO = @CD_PLANO       AND FF.CD_TIPO_CONTRIBUICAO IN ('01', '59')       AND FF.ANO_COMP * 13 + FF.MES_COMP = (SELECT MAX(FF2.ANO_COMP * 13 + FF2.MES_COMP)                                                                                    FROM CC_FICHA_FINANCEIRA FF2 										WHERE FF2.CD_FUNDACAO = FF.CD_FUNDACAO                                             										  AND FF2.NUM_INSCRICAO = FF.NUM_INSCRICAO                                             										  AND FF2.CD_PLANO = FF.CD_PLANO  										   										  AND FF2.CD_TIPO_CONTRIBUICAO IN ('01', '59')     										  AND FF2.MES_COMP <> 13) ORDER BY FF.CD_TIPO_CONTRIBUICAO", new { CD_FUNDACAO, NUM_INSCRICAO, CD_PLANO }, Transaction).ToList();
				else if (AppSettings.IS_ORACLE_PROVIDER)
					return Conexao.Query<FichaFinanceiraEntidade>("SELECT TC.DS_TIPO_CONTRIBUICAO, FF.* FROM CC_FICHA_FINANCEIRA  FF  INNER  JOIN TB_TIPO_CONTRIBUICAO   TC  ON TC.CD_TIPO_CONTRIBUICAO=FF.CD_TIPO_CONTRIBUICAO WHERE FF.CD_FUNDACAO=:CD_FUNDACAO AND FF.NUM_INSCRICAO=:NUM_INSCRICAO AND FF.CD_PLANO=:CD_PLANO AND FF.CD_TIPO_CONTRIBUICAO IN ('01', '59') AND FF.ANO_COMP*", new { CD_FUNDACAO, NUM_INSCRICAO, CD_PLANO }, Transaction).ToList();
				else
					throw new Exception("Provider não suportado!");
		...
		public virtual List<FichaFinanceiraEntidade> BuscarContribuicoesDeficit(string CD_FUNDACAO, string CD_PLANO, string NUM_INSCRICAO)
		{
			...
			{
				if (AppSettings.IS_SQL_SERVER_PROVIDER)
					return Conexao.Query<FichaFinanceiraEntidade>("SELECT FF.CD_TIPO_CONTRIBUICAO,        TC.DS_TIPO_CONTRIBUICAO,        FF.* FROM CC_FICHA_FINANCEIRA FF   INNER JOIN TB_TIPO_CONTRIBUICAO TC ON TC.CD_TIPO_CONTRIBUICAO = FF.CD_TIPO_CONTRIBUICAO   INNER JOIN CS_FUNCIONARIO FN ON FN.CD_FUNDACAO = FF.CD_FUNDACAO AND FN.NUM_INSCRICAO = FF.NUM_INSCRICAO WHERE FF.CD_FUNDACAO = @CD_FUNDACAO   AND FF.CD_PLANO = @CD_PLANO   AND FF.NUM_INSCRICAO = @NUM_INSCRICAO   AND FF.CD_TIPO_CONTRIBUICAO IN ('73','70','74','34','B4','B6','B5','B1','B2','B3','B7','B8','B9','C1','C5','C6','D5') ORDER BY ANO_REF DESC, MES_REF DESC", new { CD_FUNDACAO, CD_PLANO, NUM_INSCRICAO }, Transaction).ToList();
				else if (AppSettings.IS_ORACLE_PROVIDER)
					return Conexao.Query<FichaFinanceiraEntidade>("SELECT FF.CD_TIPO_CONTRIBUICAO, TC.DS_TIPO_CONTRIBUICAO, FF.* FROM CC_FICHA_FINANCEIRA  FF  INNER  JOIN TB_TIPO_CONTRIBUICAO   TC  ON TC.CD_TIPO_CONTRIBUICAO=FF.CD_TIPO_CONTRIBUICAO INNER  JOIN CS_FUNCIONARIO   FN  ON FN.CD_FUNDACAO=FF.CD_FUNDACAO AND FN.NUM_INSCRICAO=FF.NUM_INSCRICAO WHERE FF.CD_FUNDACAO=:CD_FUNDACAO AND FF.CD_PLANO=:CD_PLANO AND FF.NUM_INSCRICAO=:NUM_INSCRICAO AND FF.CD_TIPO_CONTRIBUICAO IN ('73', '70', '74', '34', 'B4', 'B6', 'B5', 'B1', 'B2', 'B3', 'B7', 'B8', 'B9', 'C1', 'C5', 'C6', 'D5') ORDER BY ANO_REF DESC, MES_REF DESC", new { CD_FUNDACAO, CD_PLANO, NUM_INSCRICAO }, Transaction).ToList();
				else
					throw new Exception("Provider não suportado!");
		...
		public virtual List<FichaFinanceiraEntidade> BuscarContribuicoesDeficitPorAno(string CD_FUNDACAO, string CD_PLANO, string NUM_INSCRICAO, string ANO_REF)
		{
			...
			{
				if (AppSettings.IS_SQL_SERVER_PROVIDER)
					return Conexao.Query<FichaFinanceiraEntidade>("SELECT FF.CD_TIPO_CONTRIBUICAO,        TC.DS_TIPO_CONTRIBUICAO,        FF.* FROM CC_FICHA_FINANCEIRA FF   INNER JOIN TB_TIPO_CONTRIBUICAO TC ON TC.CD_TIPO_CONTRIBUICAO = FF.CD_TIPO_CONTRIBUICAO   INNER JOIN CS_FUNCIONARIO FN ON FN.CD_FUNDACAO = FF.CD_FUNDACAO AND FN.NUM_INSCRICAO = FF.NUM_INSCRICAO WHERE FF.CD_FUNDACAO = @CD_FUNDACAO   AND FF.CD_PLANO = @CD_PLANO   AND FF.NUM_INSCRICAO = @NUM_INSCRICAO   AND FF.ANO_REF = @ANO_REF   AND FF.CD_TIPO_CONTRIBUICAO IN ('73','70','74','34','B4','B6','B5','B1','B2','B3','B7','B8','B9','C1','C5','C6','D5') ORDER BY ANO_REF DESC, MES_REF DESC", new { CD_FUNDACAO, CD_PLANO, NUM_INSCRICAO, ANO_REF }, Transaction).ToList();
				else if (AppSettings.IS_ORACLE_PROVIDER)
					return Conexao.Query<FichaFinanceiraEntidade>("SELECT FF.CD_TIPO_CONTRIBUICAO, TC.DS_TIPO_CONTRIBUICAO, FF.* FROM CC_FICHA_FINANCEIRA  FF  INNER  JOIN TB_TIPO_CONTRIBUICAO   TC  ON TC.CD_TIPO_CONTRIBUICAO=FF.CD_TIPO_CONTRIBUICAO INNER  JOIN CS_FUNCIONARIO   FN  ON FN.CD_FUNDACAO=FF.CD_FUNDACAO AND FN.NUM_INSCRICAO=FF.NUM_INSCRICAO WHERE FF.CD_FUNDACAO=:CD_FUNDACAO AND FF.CD_PLANO=:CD_PLANO AND FF.NUM_INSCRICAO=:NUM_INSCRICAO AND FF.ANO_REF=:ANO_REF AND FF.CD_TIPO_CONTRIBUICAO IN ('73', '70', '74', '34', 'B4', 'B6', 'B5', 'B1', 'B2', 'B3', 'B7', 'B8', 'B9', 'C1', 'C5', 'C6', 'D5') ORDER BY ANO_REF DESC, MES_REF DESC", new { CD_FUNDACAO, CD_PLANO, NUM_INSCRICAO, ANO_REF }, Transaction).ToList();
				else
					throw new Exception("Provider não suportado!");
		...
		public virtual List<FichaFinanceiraEntidade> BuscarDatasInformePorFundacaoInscricaoOrigemNaoG0(string CD_FUNDACAO, string NUM_INSCRICAO)
		{
			try
			{
				if (AppSettings.IS_SQL_SERVER_PROVIDER)
					return Conexao.Query<FichaFinanceiraEntidade>("SELECT DISTINCT FF.ANO_REF FROM CC_FICHA_FINANCEIRA FF 	INNER JOIN CS_FUNCIONARIO FN ON FN.CD_FUNDACAO = FF.CD_FUNDACAO  			AND FN.NUM_INSCRICAO = FF.NUM_INSCRICAO 	INNER JOIN TB_TIPO_CONTRIBUICAO TC ON TC.CD_TIPO_CONTRIBUICAO = FF.CD_TIPO_CONTRIBUICAO WHERE TC.CK_COMPOE_IR_AM = 'S'   AND FF.CD_FUNDACAO = @CD_FUNDACAO   AND FF.NUM_INSCRICAO = @NUM_INSCRICAO   AND FF.CD_ORIGEM NOT IN ('G0') GROUP BY FF.ANO_REF ORDER BY FF.ANO_REF DESC", new { CD_FUNDACAO, NUM_INSCRICAO }, Transaction).ToList();
				else if (AppSettings.IS_ORACLE_PROVIDER)
					return Conexao.Query<FichaFinanceiraEntidade>("SELECT DISTINCT FF.ANO_REF FROM CC_FICHA_FINANCEIRA  FF  INNER  JOIN CS_FUNCIONARIO   FN  ON FN.CD_FUNDACAO=FF.CD_FUNDACAO AND FN.NUM_INSCRICAO=FF.NUM_INSCRICAO INNER  JOIN TB_TIPO_CONTRIBUICAO   TC  ON TC.CD_TIPO_CONTRIBUICAO=FF.CD_TIPO_CONTRIBUICAO WHERE TC.CK_COMPOE_IR_AM='S' AND FF.CD_FUNDACAO=:CD_FUNDACAO AND FF.NUM_INSCRICAO=:NUM_INSCRICAO AND FF.CD_ORIGEM NOT  IN ('G0') GROUP BY FF.ANO_REF ORDER BY FF.ANO_REF DESC", new { CD_FUNDACAO, NUM_INSCRICAO }, Transaction).ToList();
				else
					throw new Exception("Provider não suportado!");
		...
		public virtual List<FichaFinanceiraEntidade> BuscarDatasPorFundacaoInscricaoPlano(string CD_FUNDACAO, string NUM_INSCRICAO, string CD_PLANO)
		{
			...
			{
				if (AppSettings.IS_SQL_SERVER_PROVIDER)
					return Conexao.Query<FichaFinanceiraEntidade>("SELECT DISTINCT         FF.ANO_REF,        FF.MES_REF FROM CC_FICHA_FINANCEIRA FF WHERE FF.CD_FUNDACAO   = @CD_FUNDACAO   AND FF.NUM_INSCRICAO = @NUM_INSCRICAO   AND FF.CD_PLANO      = @CD_PLANO ORDER BY FF.ANO_REF, FF.MES_REF", new { CD_FUNDACAO, NUM_INSCRICAO, CD_PLANO }, Transaction).ToList();
				else if (AppSettings.IS_ORACLE_PROVIDER)
					return Conexao.Query<FichaFinanceiraEntidade>("SELECT DISTINCT FF.ANO_REF, FF.MES_REF FROM CC_FICHA_FINANCEIRA  FF  WHERE FF.CD_FUNDACAO=:CD_FUNDACAO AND FF.NUM_INSCRICAO=:NUM_INSCRICAO AND FF.CD_PLANO=:CD_PLANO ORDER BY FF.ANO_REF, FF.MES_REF", new { CD_FUNDACAO, NUM_INSCRICAO, CD_PLANO }, Transaction).ToList();
				else
					throw new Exception("Provider não suportado!");
		...
		public virtual List<FichaFinanceiraEntidade> BuscarInformePlanosPorFundacaoInscricaoAno(string CD_FUNDACAO, string NUM_INSCRICAO, string ANO)
		{
			...
			{
				if (AppSettings.IS_SQL_SERVER_PROVIDER)
					return Conexao.Query<FichaFinanceiraEntidade>("SELECT FF.CD_PLANO, 	   SUM(FF.CONTRIB_PARTICIPANTE) AS CONTRIB_PARTICIPANTE, 	   SUM(FF.CONTRIB_EMPRESA) AS CONTRIB_EMPRESA FROM CC_FICHA_FINANCEIRA FF 	INNER JOIN CS_FUNCIONARIO FN ON FN.CD_FUNDACAO = FF.CD_FUNDACAO  			AND FN.NUM_INSCRICAO = FF.NUM_INSCRICAO 	INNER JOIN TB_TIPO_CONTRIBUICAO TC ON TC.CD_TIPO_CONTRIBUICAO = FF.CD_TIPO_CONTRIBUICAO WHERE TC.CK_COMPOE_IR_AM = 'S'   AND FF.CD_FUNDACAO = @CD_FUNDACAO   AND FF.NUM_INSCRICAO = @NUM_INSCRICAO   AND FF.ANO_REF = @ANO GROUP BY FF.CD_PLANO", new { CD_FUNDACAO, NUM_INSCRICAO, ANO }, Transaction).ToList();
				else if (AppSettings.IS_ORACLE_PROVIDER)
					return Conexao.Query<FichaFinanceiraEntidade>("SELECT FF.CD_PLANO, SUM(FF.CONTRIB_PARTICIPANTE) AS CONTRIB_PARTICIPANTE, SUM(FF.CONTRIB_EMPRESA) AS CONTRIB_EMPRESA FROM CC_FICHA_FINANCEIRA  FF  INNER  JOIN CS_FUNCIONARIO   FN  ON FN.CD_FUNDACAO=FF.CD_FUNDACAO AND FN.NUM_INSCRICAO=FF.NUM_INSCRICAO INNER  JOIN TB_TIPO_CONTRIBUICAO   TC  ON TC.CD_TIPO_CONTRIBUICAO=FF.CD_TIPO_CONTRIBUICAO WHERE TC.CK_COMPOE_IR_AM='S' AND FF.CD_FUNDACAO=:CD_FUNDACAO AND FF.NUM_INSCRICAO=:NUM_INSCRICAO AND FF.ANO_REF=:ANO GROUP BY FF.CD_PLANO", new { CD_FUNDACAO, NUM_INSCRICAO, ANO }, Transaction).ToList();
				else
					throw new Exception("Provider não suportado!");
		...
		public virtual List<FichaFinanceiraEntidade> BuscarInformePlanosPorFundacaoInscricaoAnoOrigemNaoCCCAAM(string CD_FUNDACAO, string NUM_INSCRICAO, string ANO)
		{
			try
			{
				if (AppSettings.IS_SQL_SERVER_PROVIDER)
					return Conexao.Query<FichaFinanceiraEntidade>("SELECT FF.CD_PLANO, 	   SUM(FF.CONTRIB_PARTICIPANTE) AS CONTRIB_PARTICIPANTE FROM CC_FICHA_FINANCEIRA FF 	INNER JOIN CS_FUNCIONARIO FN ON FN.CD_FUNDACAO = FF.CD_FUNDACAO  			AND FN.NUM_INSCRICAO = FF.NUM_INSCRICAO 	INNER JOIN TB_TIPO_CONTRIBUICAO TC ON TC.CD_TIPO_CONTRIBUICAO = FF.CD_TIPO_CONTRIBUICAO WHERE TC.CK_COMPOE_IR_AM = 'S'   AND FF.CD_FUNDACAO = @CD_FUNDACAO   AND FF.NUM_INSCRICAO = @NUM_INSCRICAO   AND FF.ANO_REF = @ANO   AND FF.CD_ORIGEM NOT IN ('CC', 'CA', 'AM') GROUP BY FF.CD_PLANO", new { CD_FUNDACAO, NUM_INSCRICAO, ANO }, Transaction).ToList();
				else if (AppSettings.IS_ORACLE_PROVIDER)
					return Conexao.Query<FichaFinanceiraEntidade>("SELECT FF.CD_PLANO, SUM(FF.CONTRIB_PARTICIPANTE) AS CONTRIB_PARTICIPANTE FROM CC_FICHA_FINANCEIRA  FF  INNER  JOIN CS_FUNCIONARIO   FN  ON FN.CD_FUNDACAO=FF.CD_FUNDACAO AND FN.NUM_INSCRICAO=FF.NUM_INSCRICAO INNER  JOIN TB_TIPO_CONTRIBUICAO   TC  ON TC.CD_TIPO_CONTRIBUICAO=FF.CD_TIPO_CONTRIBUICAO WHERE TC.CK_COMPOE_IR_AM='S' AND FF.CD_FUNDACAO=:CD_FUNDACAO AND FF.NUM_INSCRICAO=:NUM_INSCRICAO AND FF.ANO_REF=:ANO AND FF.CD_ORIGEM NOT  IN ('CC', 'CA', 'AM') GROUP BY FF.CD_PLANO", new { CD_FUNDACAO, NUM_INSCRICAO, ANO }, Transaction).ToList();
				else
					throw new Exception("Provider não suportado!");
		...
		public virtual List<FichaFinanceiraEntidade> BuscarInformePlanosPorFundacaoInscricaoAnoOrigemNaoG0(string CD_FUNDACAO, string NUM_INSCRICAO, string ANO)
		{
			try
			{
				if (AppSettings.IS_SQL_SERVER_PROVIDER)
					return Conexao.Query<FichaFinanceiraEntidade>("SELECT FF.CD_PLANO, 	   SUM(FF.CONTRIB_PARTICIPANTE) AS CONTRIB_PARTICIPANTE, 	   SUM(FF.CONTRIB_EMPRESA) AS CONTRIB_EMPRESA FROM CC_FICHA_FINANCEIRA FF 	INNER JOIN CS_FUNCIONARIO FN ON FN.CD_FUNDACAO = FF.CD_FUNDACAO  			AND FN.NUM_INSCRICAO = FF.NUM_INSCRICAO 	INNER JOIN TB_TIPO_CONTRIBUICAO TC ON TC.CD_TIPO_CONTRIBUICAO = FF.CD_TIPO_CONTRIBUICAO WHERE TC.CK_COMPOE_IR_AM = 'S'   AND FF.CD_FUNDACAO = @CD_FUNDACAO   AND FF.NUM_INSCRICAO = @NUM_INSCRICAO   AND FF.ANO_REF = @ANO   AND FF.CD_ORIGEM NOT IN ('G0') GROUP BY FF.CD_PLANO", new { CD_FUNDACAO, NUM_INSCRICAO, ANO }, Transaction).ToList();
				else if (AppSettings.IS_ORACLE_PROVIDER)
					return Conexao.Query<FichaFinanceiraEntidade>("SELECT FF.CD_PLANO, SUM(FF.CONTRIB_PARTICIPANTE) AS CONTRIB_PARTICIPANTE, SUM(FF.CONTRIB_EMPRESA) AS CONTRIB_EMPRESA FROM CC_FICHA_FINANCEIRA  FF  INNER  JOIN CS_FUNCIONARIO   FN  ON FN.CD_FUNDACAO=FF.CD_FUNDACAO AND FN.NUM_INSCRICAO=FF.NUM_INSCRICAO INNER  JOIN TB_TIPO_CONTRIBUICAO   TC  ON TC.CD_TIPO_CONTRIBUICAO=FF.CD_TIPO_CONTRIBUICAO WHERE TC.CK_COMPOE_IR_AM='S' AND FF.CD_FUNDACAO=:CD_FUNDACAO AND FF.NUM_INSCRICAO=:NUM_INSCRICAO AND FF.ANO_REF=:ANO AND FF.CD_ORIGEM NOT  IN ('G0') GROUP BY FF.CD_PLANO", new { CD_FUNDACAO, NUM_INSCRICAO, ANO }, Transaction).ToList();
				else
					throw new Exception("Provider não suportado!");
		...
		public virtual List<FichaFinanceiraEntidade> BuscarPlanoUmDoisPorFundacaoInscricao(string CD_FUNDACAO, string NUM_INSCRICAO)
		{
			...
			{
				if (AppSettings.IS_SQL_SERVER_PROVIDER)
					return Conexao.Query<FichaFinanceiraEntidade>("SELECT CC_FICHA_FINANCEIRA.* FROM CC_FICHA_FINANCEIRA INNER JOIN TB_TIPO_CONTRIBUICAO ON TB_TIPO_CONTRIBUICAO.CD_TIPO_CONTRIBUICAO = CC_FICHA_FINANCEIRA.CD_TIPO_CONTRIBUICAO WHERE (CC_FICHA_FINANCEIRA.CD_FUNDACAO = @CD_FUNDACAO)   AND (CC_FICHA_FINANCEIRA.NUM_INSCRICAO = @NUM_INSCRICAO)   AND (CC_FICHA_FINANCEIRA.CD_PLANO IN ('0001', '0002'))   AND (TB_TIPO_CONTRIBUICAO.CALC_MARGEM_CONSIG = 'S')", new { CD_FUNDACAO, NUM_INSCRICAO }, Transaction).ToList();
				else if (AppSettings.IS_ORACLE_PROVIDER)
					return Conexao.Query<FichaFinanceiraEntidade>("SELECT CC_FICHA_FINANCEIRA.* FROM CC_FICHA_FINANCEIRA INNER  JOIN TB_TIPO_CONTRIBUICAO  ON TB_TIPO_CONTRIBUICAO.CD_TIPO_CONTRIBUICAO=CC_FICHA_FINANCEIRA.CD_TIPO_CONTRIBUICAO WHERE (CC_FICHA_FINANCEIRA.CD_FUNDACAO=:CD_FUNDACAO) AND (CC_FICHA_FINANCEIRA.NUM_INSCRICAO=:NUM_INSCRICAO) AND (CC_FICHA_FINANCEIRA.CD_PLANO IN ('0001', '0002')) AND (TB_TIPO_CONTRIBUICAO.CALC_MARGEM_CONSIG='S')", new { CD_FUNDACAO, NUM_INSCRICAO }, Transaction).ToList();
				else
					throw new Exception("Provider não suportado!");
		...
		public virtual List<FichaFinanceiraEntidade> BuscarPorFundacaoPlanoInscricao(string CD_FUNDACAO, string CD_PLANO, string NUM_INSCRICAO)
		{
			...
			{
				if (AppSettings.IS_SQL_SERVER_PROVIDER)
					return Conexao.Query<FichaFinanceiraEntidade>("SELECT TC.DS_TIPO_CONTRIBUICAO,        TC.CALC_MARGEM_CONSIG,        FI.*  FROM CC_FICHA_FINANCEIRA FI INNER JOIN TB_TIPO_CONTRIBUICAO TC ON TC.CD_TIPO_CONTRIBUICAO = FI.CD_TIPO_CONTRIBUICAO WHERE FI.CD_FUNDACAO = @CD_FUNDACAO   AND FI.CD_PLANO = @CD_PLANO   AND FI.NUM_INSCRICAO = @NUM_INSCRICAO ORDER BY FI.ANO_REF DESC,           FI.MES_REF DESC,          FI.CD_TIPO_CONTRIBUICAO", new { CD_FUNDACAO, CD_PLANO, NUM_INSCRICAO }, Transaction).ToList();
				else if (AppSettings.IS_ORACLE_PROVIDER)
					return Conexao.Query<FichaFinanceiraEntidade>("SELECT TC.DS_TIPO_CONTRIBUICAO, TC.CALC_MARGEM_CONSIG, FI.* FROM CC_FICHA_FINANCEIRA  FI  INNER  JOIN TB_TIPO_CONTRIBUICAO   TC  ON TC.CD_TIPO_CONTRIBUICAO=FI.CD_TIPO_CONTRIBUICAO WHERE FI.CD_FUNDACAO=:CD_FUNDACAO AND FI.CD_PLANO=:CD_PLANO AND FI.NUM_INSCRICAO=:NUM_INSCRICAO ORDER BY FI.ANO_REF DESC, FI.MES_REF DESC, FI.CD_TIPO_CONTRIBUICAO", new { CD_FUNDACAO, CD_PLANO, NUM_INSCRICAO }, Transaction).ToList();
				else
					throw new Exception("Provider não suportado!");
		...
		public virtual List<FichaFinanceiraEntidade> BuscarPorFundacaoPlanoInscricaoPeriodo(string CD_FUNDACAO, string CD_PLANO, string NUM_INSCRICAO, DateTime DT_INICIO, DateTime DT_FIM)
		{
			...
			{
				if (AppSettings.IS_SQL_SERVER_PROVIDER)
					return Conexao.Query<FichaFinanceiraEntidade>("SELECT TC.DS_TIPO_CONTRIBUICAO,        TC.CALC_MARGEM_CONSIG,        FI.*  FROM CC_FICHA_FINANCEIRA FI INNER JOIN TB_TIPO_CONTRIBUICAO TC ON TC.CD_TIPO_CONTRIBUICAO = FI.CD_TIPO_CONTRIBUICAO WHERE FI.CD_FUNDACAO = @CD_FUNDACAO   AND FI.CD_PLANO = @CD_PLANO   AND FI.NUM_INSCRICAO = @NUM_INSCRICAO   AND '' + FI.ANO_REF + '-' + FI.MES_REF + '-01' BETWEEN @DT_INICIO AND @DT_FIM  ORDER BY FI.ANO_REF DESC,           FI.MES_REF DESC,          FI.CD_TIPO_CONTRIBUICAO", new { CD_FUNDACAO, CD_PLANO, NUM_INSCRICAO, DT_INICIO, DT_FIM }, Transaction).ToList();
				else if (AppSettings.IS_ORACLE_PROVIDER)
					return Conexao.Query<FichaFinanceiraEntidade>("SELECT TC.DS_TIPO_CONTRIBUICAO, TC.CALC_MARGEM_CONSIG, FI.* FROM CC_FICHA_FINANCEIRA  FI  INNER  JOIN TB_TIPO_CONTRIBUICAO   TC  ON TC.CD_TIPO_CONTRIBUICAO=FI.CD_TIPO_CONTRIBUICAO WHERE FI.CD_FUNDACAO=:CD_FUNDACAO AND FI.CD_PLANO=:CD_PLANO AND FI.NUM_INSCRICAO=:NUM_INSCRICAO AND '' || FI.ANO_REF || '-' || FI.MES_REF || '-01' BETWEEN :DT_INICIO AND :DT_FIM ORDER BY FI.ANO_REF DESC, FI.MES_REF DESC, FI.CD_TIPO_CONTRIBUICAO", new { CD_FUNDACAO, CD_PLANO, NUM_INSCRICAO, DT_INICIO, DT_FIM }, Transaction).ToList();
				else
					throw new Exception("Provider não suportado!");
		...
		public virtual List<FichaFinanceiraEntidade> BuscarPorFundacaoPlanoInscricaoPeriodoTipoResgate(string CD_FUNDACAO, string CD_PLANO, string NUM_INSCRICAO, string ANOMES_INICIO, string ANOMES_FIM, string CD_TIPO_RESGATE)
		{
			...
			{
				if (AppSettings.IS_SQL_SERVER_PROVIDER)
					return Conexao.Query<FichaFinanceiraEntidade>("SELECT TC.DS_TIPO_CONTRIBUICAO,        TC.CALC_MARGEM_CONSIG,        FI.*  FROM CC_FICHA_FINANCEIRA FI INNER JOIN TB_TIPO_CONTRIBUICAO TC ON TC.CD_TIPO_CONTRIBUICAO = FI.CD_TIPO_CONTRIBUICAO WHERE FI.CD_FUNDACAO = @CD_FUNDACAO   AND FI.CD_PLANO = @CD_PLANO   AND FI.NUM_INSCRICAO = @NUM_INSCRICAO   AND '' + FI.ANO_REF + FI.MES_REF BETWEEN @ANOMES_INICIO AND @ANOMES_FIM    AND FI.CD_TIPO_CONTRIBUICAO IN (SELECT CD_TIPO_CONTRIBUICAO                                   FROM DR_PARAMETROS_RESGATE                                  WHERE CD_FUNDACAO = FI.CD_FUNDACAO                                     AND CD_PLANO = FI.CD_PLANO                                     AND CD_TIPO_CONTRIBUICAO = FI.CD_TIPO_CONTRIBUICAO                                     AND CD_TIPO_RESGATE = @CD_TIPO_RESGATE) ORDER BY FI.ANO_REF DESC,           FI.MES_REF DESC,          FI.CD_TIPO_CONTRIBUICAO", new { CD_FUNDACAO, CD_PLANO, NUM_INSCRICAO, ANOMES_INICIO, ANOMES_FIM, CD_TIPO_RESGATE }, Transaction).ToList();
				else if (AppSettings.IS_ORACLE_PROVIDER)
					return Conexao.Query<FichaFinanceiraEntidade>("SELECT TC.DS_TIPO_CONTRIBUICAO, TC.CALC_MARGEM_CONSIG, FI.* FROM CC_FICHA_FINANCEIRA  FI  INNER  JOIN TB_TIPO_CONTRIBUICAO   TC  ON TC.CD_TIPO_CONTRIBUICAO=FI.CD_TIPO_CONTRIBUICAO WHERE FI.CD_FUNDACAO=:CD_FUNDACAO AND FI.CD_PLANO=:CD_PLANO AND FI.NUM_INSCRICAO=:NUM_INSCRICAO AND '' || FI.ANO_REF || FI.MES_REF BETWEEN :ANOMES_INICIO AND :ANOMES_FIM AND FI.CD_TIPO_CONTRIBUICAO IN (SELECT CD_TIPO_CONTRIBUICAO FROM DR_PARAMETROS_RESGATE WHERE CD_FUNDACAO=FI.CD_FUNDACAO AND CD_PLANO=FI.CD_PLANO AND CD_TIPO_CONTRIBUICAO=FI.CD_TIPO_CONTRIBUICAO AND CD_TIPO_RESGATE=:CD_TIPO_RESGATE) ORDER BY FI.ANO_REF DESC, FI.MES_REF DESC, FI.CD_TIPO_CONTRIBUICAO", new { CD_FUNDACAO, CD_PLANO, NUM_INSCRICAO, ANOMES_INICIO, ANOMES_FIM, CD_TIPO_RESGATE }, Transaction).ToList();
				else
					throw new Exception("Provider não suportado!");
		...
		public virtual List<FichaFinanceiraEntidade> BuscarResumoAnosPorFundacaoPlanoInscricao(string CD_FUNDACAO, string CD_PLANO, string NUM_INSCRICAO)
		{
			...
			{
				if (AppSettings.IS_SQL_SERVER_PROVIDER)
					return Conexao.Query<FichaFinanceiraEntidade>("SELECT *  FROM CC_FICHA_FINANCEIRA FI WHERE FI.CD_FUNDACAO = @CD_FUNDACAO   AND FI.CD_PLANO = @CD_PLANO   AND FI.NUM_INSCRICAO = @NUM_INSCRICAO ORDER BY FI.ANO_REF DESC", new { CD_FUNDACAO, CD_PLANO, NUM_INSCRICAO }, Transaction).ToList();
				else if (AppSettings.IS_ORACLE_PROVIDER)
					return Conexao.Query<FichaFinanceiraEntidade>("SELECT * FROM CC_FICHA_FINANCEIRA  FI  WHERE FI.CD_FUNDACAO=:CD_FUNDACAO AND FI.CD_PLANO=:CD_PLANO AND FI.NUM_INSCRICAO=:NUM_INSCRICAO ORDER BY FI.ANO_REF DESC", new { CD_FUNDACAO, CD_PLANO, NUM_INSCRICAO }, Transaction).ToList();
				else
					throw new Exception("Provider não suportado!");
		...
		public virtual List<FichaFinanceiraEntidade> BuscarResumoMesesPorFundacaoPlanoInscricaoAno(string CD_FUNDACAO, string CD_PLANO, string NUM_INSCRICAO, string ANO_REF)
		{
			...
			{
				if (AppSettings.IS_SQL_SERVER_PROVIDER)
					return Conexao.Query<FichaFinanceiraEntidade>("SELECT *  FROM CC_FICHA_FINANCEIRA FI WHERE FI.CD_FUNDACAO = @CD_FUNDACAO   AND FI.CD_PLANO = @CD_PLANO   AND FI.NUM_INSCRICAO = @NUM_INSCRICAO   AND FI.ANO_REF = @ANO_REF ORDER BY FI.MES_REF", new { CD_FUNDACAO, CD_PLANO, NUM_INSCRICAO, ANO_REF }, Transaction).ToList();
				else if (AppSettings.IS_ORACLE_PROVIDER)
					return Conexao.Query<FichaFinanceiraEntidade>("SELECT * FROM CC_FICHA_FINANCEIRA  FI  WHERE FI.CD_FUNDACAO=:CD_FUNDACAO AND FI.CD_PLANO=:CD_PLANO AND FI.NUM_INSCRICAO=:NUM_INSCRICAO AND FI.ANO_REF=:ANO_REF ORDER BY FI.MES_REF", new { CD_FUNDACAO, CD_PLANO, NUM_INSCRICAO, ANO_REF }, Transaction).ToList();
				else
					throw new Exception("Provider não suportado!");
		...
		public virtual List<FichaFinanceiraEntidade> BuscarSaldosPreves(string CD_FUNDACAO, string NUM_INSCRICAO, string CD_PLANO)
		{
			try
			{
				if (AppSettings.IS_SQL_SERVER_PROVIDER)
					return Conexao.Query<FichaFinanceiraEntidade>("SELECT SUM(CASE TC.IND_PORTABILIDADE WHEN 'S' THEN 0 ELSE CASE WHEN FF.CD_OPERACAO = 'D' THEN FF.CONTRIB_PARTICIPANTE * -1 ELSE FF.CONTRIB_PARTICIPANTE END END) AS CONTRIB_PARTICIPANTE,        SUM(CASE TC.IND_PORTABILIDADE WHEN 'S' THEN 0 ELSE CASE WHEN FF.CD_OPERACAO = 'D' THEN FF.CONTRIB_EMPRESA * -1 ELSE FF.CONTRIB_EMPRESA END END) AS CONTRIB_EMPRESA,        SUM(CASE TC.IND_PORTABILIDADE WHEN 'S' THEN 0 ELSE CASE WHEN FF.CD_OPERACAO = 'D' THEN ISNULL(FF.QTD_COTA_RP_PARTICIPANTE,0) * -1 ELSE ISNULL(FF.QTD_COTA_RP_PARTICIPANTE,0) END END) AS QTD_COTA_RP_PARTICIPANTE,        SUM(CASE TC.IND_PORTABILIDADE WHEN 'S' THEN 0 ELSE CASE WHEN FF.CD_OPERACAO = 'D' THEN ISNULL(FF.QTD_COTA_RP_EMPRESA,0) * -1 ELSE ISNULL(FF.QTD_COTA_RP_EMPRESA,0) END END) AS QTD_COTA_RP_EMPRESA,        SUM(CASE TC.IND_PORTABILIDADE WHEN 'S' THEN CASE WHEN FF.CD_OPERACAO = 'D' THEN FF.CONTRIB_PARTICIPANTE * -1 ELSE FF.CONTRIB_PARTICIPANTE END ELSE 0 END) AS CONTRIB_PORTABILIDADE,        SUM(CASE TC.IND_PORTABILIDADE WHEN 'S' THEN CASE WHEN FF.CD_OPERACAO = 'D' THEN FF.QTD_COTA_RP_PARTICIPANTE * -1 ELSE FF.QTD_COTA_RP_PARTICIPANTE END ELSE 0 END) AS QTD_COTA_RP_PORTABILIDADE FROM CC_FICHA_FINANCEIRA FF     INNER JOIN TB_TIPO_CONTRIBUICAO TC ON TC.CD_TIPO_CONTRIBUICAO = FF.CD_TIPO_CONTRIBUICAO WHERE FF.CD_FUNDACAO   = @CD_FUNDACAO   AND FF.NUM_INSCRICAO = @NUM_INSCRICAO   AND FF.CD_PLANO      = @CD_PLANO   AND TC.COMPOE_SALDO_BENEFICIO = 'S'", new { CD_FUNDACAO, NUM_INSCRICAO, CD_PLANO }, Transaction).ToList();
				else if (AppSettings.IS_ORACLE_PROVIDER)
					return Conexao.Query<FichaFinanceiraEntidade>("", new { CD_FUNDACAO, NUM_INSCRICAO, CD_PLANO }, Transaction).ToList();
				else
					throw new Exception("Provider não suportado!");
		...
		public virtual List<FichaFinanceiraEntidade> BuscarSaldosSemRecursoPortadoPorInscricao(string CD_FUNDACAO, string NUM_INSCRICAO, string CD_PLANO)
		{
			...
			{
				if (AppSettings.IS_SQL_SERVER_PROVIDER)
					return Conexao.Query<FichaFinanceiraEntidade>("SELECT SUM(CASE WHEN FF.CD_OPERACAO = 'D' THEN FF.CONTRIB_PARTICIPANTE * -1 ELSE FF.CONTRIB_PARTICIPANTE END) AS CONTRIB_PARTICIPANTE,        SUM(CASE WHEN FF.CD_OPERACAO = 'D' THEN FF.CONTRIB_EMPRESA * -1 ELSE FF.CONTRIB_EMPRESA END) AS CONTRIB_EMPRESA,        SUM(CASE WHEN FF.CD_OPERACAO = 'D' THEN ISNULL(FF.QTD_COTA_RP_PARTICIPANTE,0) * -1 ELSE ISNULL(FF.QTD_COTA_RP_PARTICIPANTE,0) END) AS QTD_COTA_RP_PARTICIPANTE,        SUM(CASE WHEN FF.CD_OPERACAO = 'D' THEN ISNULL(FF.QTD_COTA_RP_EMPRESA,0) * -1 ELSE ISNULL(FF.QTD_COTA_RP_EMPRESA,0) END) AS QTD_COTA_RP_EMPRESA FROM CC_FICHA_FINANCEIRA FF     INNER JOIN TB_TIPO_CONTRIBUICAO TC ON TC.CD_TIPO_CONTRIBUICAO = FF.CD_TIPO_CONTRIBUICAO WHERE FF.CD_FUNDACAO   = @CD_FUNDACAO   AND FF.NUM_INSCRICAO = @NUM_INSCRICAO   AND FF.CD_PLANO      = @CD_PLANO   AND TC.COMPOE_SALDO_BENEFICIO = 'S'   AND TC.RECURSO_PORTADO = 'N'", new { CD_FUNDACAO, NUM_INSCRICAO, CD_PLANO }, Transaction).ToList();
				else if (AppSettings.IS_ORACLE_PROVIDER)
					return Conexao.Query<FichaFinanceiraEntidade>("", new { CD_FUNDACAO, NUM_INSCRICAO, CD_PLANO }, Transaction).ToList();
				else
					throw new Exception("Provider não suportado!");
