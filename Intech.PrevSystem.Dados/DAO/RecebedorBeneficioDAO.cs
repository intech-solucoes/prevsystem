...
namespace Intech.PrevSystem.Dados.DAO
{
	...
	{
		public RecebedorBeneficioDAO (IDbTransaction tx = null) : base(tx) { }
		...
		public virtual RecebedorBeneficioEntidade BuscarPorFundacaoEmpresaInscricao(string CD_FUNDACAO, string CD_EMPRESA, string NUM_INSCRICAO)
		{
			...
			{
				if (AppSettings.IS_SQL_SERVER_PROVIDER)
					return Conexao.QuerySingleOrDefault<RecebedorBeneficioEntidade>("SELECT RB.* FROM GB_RECEBEDOR_BENEFICIO RB     INNER JOIN GB_PROCESSOS_BENEFICIO PB ON PB.CD_FUNDACAO = RB.CD_FUNDACAO         AND PB.NUM_INSCRICAO = RB.NUM_INSCRICAO     INNER JOIN GB_ESPECIE_BENEFICIO EB ON EB.CD_ESPECIE = PB.CD_ESPECIE WHERE PB.DT_TERMINO >= GETDATE()   AND RB.CD_FUNDACAO = @CD_FUNDACAO   AND RB.CD_EMPRESA = @CD_EMPRESA   AND RB.NUM_INSCRICAO = @NUM_INSCRICAO", new { CD_FUNDACAO, CD_EMPRESA, NUM_INSCRICAO }, Transaction);
				else if (AppSettings.IS_ORACLE_PROVIDER)
					return Conexao.QuerySingleOrDefault<RecebedorBeneficioEntidade>("SELECT RB.* FROM GB_RECEBEDOR_BENEFICIO  RB  INNER  JOIN GB_PROCESSOS_BENEFICIO   PB  ON PB.CD_FUNDACAO=RB.CD_FUNDACAO AND PB.NUM_INSCRICAO=RB.NUM_INSCRICAO INNER  JOIN GB_ESPECIE_BENEFICIO   EB  ON EB.CD_ESPECIE=PB.CD_ESPECIE WHERE PB.DT_TERMINO>=SYSDATE AND RB.CD_FUNDACAO=:CD_FUNDACAO AND RB.CD_EMPRESA=:CD_EMPRESA AND RB.NUM_INSCRICAO=:NUM_INSCRICAO", new { CD_FUNDACAO, CD_EMPRESA, NUM_INSCRICAO }, Transaction);
				else
					throw new Exception("Provider não suportado!");
			}
			finally
		...
		public virtual List<RecebedorBeneficioEntidade> BuscarPorFundacaoEmpresaInscricaoSemJoins(string CD_FUNDACAO, string CD_EMPRESA, string NUM_INSCRICAO)
		{
			...
			{
				if (AppSettings.IS_SQL_SERVER_PROVIDER)
					return Conexao.Query<RecebedorBeneficioEntidade>("SELECT RB.* FROM GB_RECEBEDOR_BENEFICIO RB WHERE RB.CD_FUNDACAO = @CD_FUNDACAO   AND RB.CD_EMPRESA = @CD_EMPRESA   AND RB.NUM_INSCRICAO = @NUM_INSCRICAO", new { CD_FUNDACAO, CD_EMPRESA, NUM_INSCRICAO }, Transaction).ToList();
				else if (AppSettings.IS_ORACLE_PROVIDER)
					return Conexao.Query<RecebedorBeneficioEntidade>("SELECT RB.* FROM GB_RECEBEDOR_BENEFICIO  RB  WHERE RB.CD_FUNDACAO=:CD_FUNDACAO AND RB.CD_EMPRESA=:CD_EMPRESA AND RB.NUM_INSCRICAO=:NUM_INSCRICAO", new { CD_FUNDACAO, CD_EMPRESA, NUM_INSCRICAO }, Transaction).ToList();
				else
					throw new Exception("Provider não suportado!");
