    ...
    public abstract class FatorRiscoDAO : BaseDAO<FatorRiscoEntidade>
    {
		public virtual FatorRiscoEntidade BuscarPorFundacaoPlano(string CD_FUNDACAO, string CD_PLANO, int IDADE)
		{
			if(AppSettings.IS_SQL_SERVER_PROVIDER)
				...
				return Conexao.QuerySingleOrDefault<FatorRiscoEntidade>("SELECT * FROM WEB_FATOR_RISCO WHERE CD_FUNDACAO=:CD_FUNDACAO AND CD_PLANO=:CD_PLANO AND :IDADE BETWEEN NUM_FAIXA_INI AND NUM_FAIXA_FIMAND ROWNUM <= 1 ", new { CD_FUNDACAO, CD_PLANO, IDADE });
			else
