...
namespace Intech.PrevSystem.Dados.DAO
{
	public abstract class FaixasIRRFDAO : BaseDAO<FaixasIRRFEntidade>
	{
		...
		public virtual List<FaixasIRRFEntidade> BuscarPorDataReferenciaTipo(DateTime DT_REFERENCIA, decimal TIPO_IRRF)
		{
			try
			{
				if (AppSettings.IS_SQL_SERVER_PROVIDER)
					return Conexao.Query<FaixasIRRFEntidade>("SELECT * FROM   TB_FAIXAS_IRRF WHERE ( DT_REFERENCIA = @DT_REFERENCIA )   AND ( TIPO_IRRF = @TIPO_IRRF ) ORDER  BY FAIXA_IRRF", new { DT_REFERENCIA, TIPO_IRRF }, Transaction).ToList();
