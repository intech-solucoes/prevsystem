...
SELECT GB_PROCESSOS_BENEFICIO.*,
       ...
       GB_HIST_RENDAS.VL_PARCELA_MENSAL
...
LEFT OUTER JOIN GB_HIST_RENDAS ON GB_PROCESSOS_BENEFICIO.CD_FUNDACAO = GB_HIST_RENDAS.CD_FUNDACAO 
    AND GB_PROCESSOS_BENEFICIO.CD_EMPRESA = GB_HIST_RENDAS.CD_EMPRESA 
    AND GB_PROCESSOS_BENEFICIO.CD_PLANO = GB_HIST_RENDAS.CD_PLANO 
    AND GB_PROCESSOS_BENEFICIO.CD_ESPECIE = GB_HIST_RENDAS.CD_ESPECIE 
    AND GB_PROCESSOS_BENEFICIO.NUM_PROCESSO = GB_HIST_RENDAS.NUM_PROCESSO 
    AND GB_PROCESSOS_BENEFICIO.ANO_PROCESSO = GB_HIST_RENDAS.ANO_PROCESSO 
WHERE  CS_FUNCIONARIO.CD_FUNDACAO = @CD_FUNDACAO
   ...
   AND GB_ESPECIE_BENEFICIO.CD_GRUPO_ESPECIE IN ('1', '3')
    AND GB_HIST_RENDAS.DT_INIC_VALIDADE = (SELECT MAX(HR.DT_INIC_VALIDADE)
                        FROM GB_HIST_RENDAS HR WHERE GB_PROCESSOS_BENEFICIO.CD_FUNDACAO = HR.CD_FUNDACAO 
                            AND GB_PROCESSOS_BENEFICIO.CD_EMPRESA = HR.CD_EMPRESA 
                            AND GB_PROCESSOS_BENEFICIO.CD_PLANO = HR.CD_PLANO 
                            AND GB_PROCESSOS_BENEFICIO.CD_ESPECIE = HR.CD_ESPECIE 
                            AND GB_PROCESSOS_BENEFICIO.NUM_PROCESSO = HR.NUM_PROCESSO 
                            AND GB_PROCESSOS_BENEFICIO.ANO_PROCESSO = HR.ANO_PROCESSO)
