...
{   
    public abstract class FichaFechamentoPrevesDAO : BaseDAO<FichaFechamentoPrevesEntidade>
    {
		...
		public virtual IEnumerable<FichaFechamentoPrevesEntidade> BuscarResumoAnosPorFundacaoEmpresaPlanoInscricao(string CD_FUNDACAO, string CD_EMPRESA, string CD_PLANO, string NUM_INSCRICAO)
		{
			...
			{
				if(AppSettings.IS_SQL_SERVER_PROVIDER)
					return Conexao.Query<FichaFechamentoPrevesEntidade>("SELECT FF.*, 	LO.DS_LOTACAO FROM CC_FICHA_FECHAMENTO_PREVES FF INNER JOIN CS_FUNCIONARIO FUNC ON FUNC.NUM_INSCRICAO = FF.NUM_INSCRICAO INNER JOIN TB_LOTACAO LO ON LO.CD_LOTACAO = FUNC.CD_LOTACAO 						AND LO.CD_EMPRESA = FUNC.CD_EMPRESA WHERE FF.CD_FUNDACAO = @CD_FUNDACAO   AND FF.CD_EMPRESA = @CD_EMPRESA   AND FF.CD_PLANO = @CD_PLANO   AND FF.NUM_INSCRICAO = @NUM_INSCRICAO   AND FF.IND_ANALITICO_SINTETICO = 'S' ORDER BY FF.ANO_REF,          FF.ANO_COMP", new { CD_FUNDACAO, CD_EMPRESA, CD_PLANO, NUM_INSCRICAO });
				else if(AppSettings.IS_ORACLE_PROVIDER)
					return Conexao.Query<FichaFechamentoPrevesEntidade>("SELECT FF.*, LO.DS_LOTACAO FROM CC_FICHA_FECHAMENTO_PREVES  FF  INNER  JOIN CS_FUNCIONARIO   FUNC  ON FUNC.NUM_INSCRICAO=FF.NUM_INSCRICAO INNER  JOIN TB_LOTACAO   LO  ON LO.CD_LOTACAO=FUNC.CD_LOTACAO AND LO.CD_EMPRESA=FUNC.CD_EMPRESA WHERE FF.CD_FUNDACAO=:CD_FUNDACAO AND FF.CD_EMPRESA=:CD_EMPRESA AND FF.CD_PLANO=:CD_PLANO AND FF.NUM_INSCRICAO=:NUM_INSCRICAO AND FF.IND_ANALITICO_SINTETICO='S' ORDER BY FF.ANO_REF, FF.ANO_COMP", new { CD_FUNDACAO, CD_EMPRESA, CD_PLANO, NUM_INSCRICAO });
				else
					throw new Exception("Provider não suportado!");
		...
		public virtual FichaFechamentoPrevesEntidade BuscarUltimaPorFundacaoEmpresaPlanoInscricaoTipo(string CD_FUNDACAO, string CD_EMPRESA, string CD_PLANO, string NUM_INSCRICAO, string TIPO)
		{
			try
			{
				if(AppSettings.IS_SQL_SERVER_PROVIDER)
					return Conexao.QuerySingleOrDefault<FichaFechamentoPrevesEntidade>("SELECT TOP 1  	FF.*, 	LO.DS_LOTACAO FROM CC_FICHA_FECHAMENTO_PREVES FF INNER JOIN CS_FUNCIONARIO FUNC ON FUNC.NUM_INSCRICAO = FF.NUM_INSCRICAO INNER JOIN TB_LOTACAO LO ON LO.CD_LOTACAO = FUNC.CD_LOTACAO 						AND LO.CD_EMPRESA = FUNC.CD_EMPRESA WHERE FF.CD_FUNDACAO = @CD_FUNDACAO   AND FF.CD_EMPRESA = @CD_EMPRESA   AND FF.CD_PLANO = @CD_PLANO   AND FF.NUM_INSCRICAO = @NUM_INSCRICAO   AND FF.IND_ANALITICO_SINTETICO = @TIPO ORDER BY FF.CD_FUNDACAO,          FF.CD_EMPRESA,          FF.NUM_INSCRICAO,          FF.ANO_REF DESC,          FF.MES_REF DESC,          FF.ANO_COMP DESC,          FF.MES_COMP DESC,          FF.NUM_SEQ,          FF.IND_TIPO", new { CD_FUNDACAO, CD_EMPRESA, CD_PLANO, NUM_INSCRICAO, TIPO });
				else if(AppSettings.IS_ORACLE_PROVIDER)
					return Conexao.QuerySingleOrDefault<FichaFechamentoPrevesEntidade>("SELECT FF.*, LO.DS_LOTACAO FROM CC_FICHA_FECHAMENTO_PREVES  FF  INNER  JOIN CS_FUNCIONARIO   FUNC  ON FUNC.NUM_INSCRICAO=FF.NUM_INSCRICAO INNER  JOIN TB_LOTACAO   LO  ON LO.CD_LOTACAO=FUNC.CD_LOTACAO AND LO.CD_EMPRESA=FUNC.CD_EMPRESA WHERE FF.CD_FUNDACAO=:CD_FUNDACAO AND FF.CD_EMPRESA=:CD_EMPRESA AND FF.CD_PLANO=:CD_PLANO AND FF.NUM_INSCRICAO=:NUM_INSCRICAO AND FF.IND_ANALITICO_SINTETICO=:TIPO AND ROWNUM <= 1  ORDER BY FF.CD_FUNDACAO, FF.CD_EMPRESA, FF.NUM_INSCRICAO, FF.ANO_REF DESC, FF.MES_REF DESC, FF.ANO_COMP DESC, FF.MES_COMP DESC, FF.NUM_SEQ, FF.IND_TIPO", new { CD_FUNDACAO, CD_EMPRESA, CD_PLANO, NUM_INSCRICAO, TIPO });
				else
					throw new Exception("Provider não suportado!");
			}
			finally
