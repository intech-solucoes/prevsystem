...
{   
    public abstract class ArquivoUploadDAO : BaseDAO<ArquivoUploadEntidade>
    {
		...
		public virtual IEnumerable<ArquivoUploadEntidade> Buscar()
		{
			try
			{
				if(AppSettings.IS_SQL_SERVER_PROVIDER)
					return Conexao.Query<ArquivoUploadEntidade>("SELECT * FROM TBG_ARQUIVO_UPLOAD", new {  });
		...
		public virtual IEnumerable<ArquivoUploadEntidade> BuscarPorNome(string NOM_ARQUIVO_ORIGINAL)
		{
			try
			{
				if(AppSettings.IS_SQL_SERVER_PROVIDER)
					return Conexao.Query<ArquivoUploadEntidade>("SELECT * FROM TBG_ARQUIVO_UPLOAD  WHERE      NOM_ARQUIVO_ORIGINAL = @NOM_ARQUIVO_ORIGINAL", new { NOM_ARQUIVO_ORIGINAL });
				else if(AppSettings.IS_ORACLE_PROVIDER)
					return Conexao.Query<ArquivoUploadEntidade>("SELECT * FROM TBG_ARQUIVO_UPLOAD WHERE NOM_ARQUIVO_ORIGINAL=:NOM_ARQUIVO_ORIGINAL", new { NOM_ARQUIVO_ORIGINAL });
				else
					throw new Exception("Provider não suportado!");
