...
namespace Intech.PrevSystem.Dados.DAO
{
	public abstract class DependenteDAO : BaseDAO<DependenteEntidade>
	{
		...
		public virtual List<DependenteEntidade> BuscarPorFundacaoInscricao(string CD_FUNDACAO, string NUM_INSCRICAO)
		{
			...
			{
				if (AppSettings.IS_SQL_SERVER_PROVIDER)
					return Conexao.Query<DependenteEntidade>("SELECT CS_DEPENDENTE.*,      TB_GRAU_PARENTESCO.DS_GRAU_PARENTESCO,      CS_MOT_PERDA_VALIDADE.DS_MOT_PERDA_VALIDADE,      CS_FUNCIONARIO.NUM_MATRICULA,      CS_FUNCIONARIO.CD_EMPRESA,      EMPRESA.NOME_ENTID AS DS_EMPRESA   FROM CS_DEPENDENTE  INNER JOIN TB_GRAU_PARENTESCO ON TB_GRAU_PARENTESCO.CD_GRAU_PARENTESCO = CS_DEPENDENTE.CD_GRAU_PARENTESCO  LEFT OUTER JOIN CS_MOT_PERDA_VALIDADE ON CS_MOT_PERDA_VALIDADE.CD_MOT_PERDA_VALIDADE = CS_DEPENDENTE.CD_MOT_PERDA_VALIDADE  INNER JOIN CS_FUNCIONARIO ON CS_FUNCIONARIO.NUM_INSCRICAO = CS_DEPENDENTE.NUM_INSCRICAO  INNER JOIN TB_EMPRESA ON TB_EMPRESA.CD_EMPRESA = CS_FUNCIONARIO.CD_EMPRESA  INNER JOIN EE_ENTIDADE EMPRESA ON EMPRESA.COD_ENTID = TB_EMPRESA.COD_ENTID  WHERE CS_DEPENDENTE.CD_FUNDACAO = @CD_FUNDACAO    AND CS_DEPENDENTE.NUM_INSCRICAO = @NUM_INSCRICAO  ORDER BY CS_DEPENDENTE.NOME_DEP", new { CD_FUNDACAO, NUM_INSCRICAO }, Transaction).ToList();
				else if (AppSettings.IS_ORACLE_PROVIDER)
					return Conexao.Query<DependenteEntidade>("SELECT CS_DEPENDENTE.*, TB_GRAU_PARENTESCO.DS_GRAU_PARENTESCO, CS_MOT_PERDA_VALIDADE.DS_MOT_PERDA_VALIDADE, CS_FUNCIONARIO.NUM_MATRICULA, CS_FUNCIONARIO.CD_EMPRESA, EMPRESA.NOME_ENTID AS DS_EMPRESA FROM CS_DEPENDENTE INNER  JOIN TB_GRAU_PARENTESCO  ON TB_GRAU_PARENTESCO.CD_GRAU_PARENTESCO=CS_DEPENDENTE.CD_GRAU_PARENTESCO LEFT OUTER JOIN CS_MOT_PERDA_VALIDADE  ON CS_MOT_PERDA_VALIDADE.CD_MOT_PERDA_VALIDADE=CS_DEPENDENTE.CD_MOT_PERDA_VALIDADE INNER  JOIN CS_FUNCIONARIO  ON CS_FUNCIONARIO.NUM_INSCRICAO=CS_DEPENDENTE.NUM_INSCRICAO INNER  JOIN TB_EMPRESA  ON TB_EMPRESA.CD_EMPRESA=CS_FUNCIONARIO.CD_EMPRESA INNER  JOIN EE_ENTIDADE   EMPRESA  ON EMPRESA.COD_ENTID=TB_EMPRESA.COD_ENTID WHERE CS_DEPENDENTE.CD_FUNDACAO=:CD_FUNDACAO AND CS_DEPENDENTE.NUM_INSCRICAO=:NUM_INSCRICAO ORDER BY CS_DEPENDENTE.NOME_DEP", new { CD_FUNDACAO, NUM_INSCRICAO }, Transaction).ToList();
				else
					throw new Exception("Provider não suportado!");
		...
		public virtual List<DependenteEntidade> BuscarPorFundacaoInscricaoDtValidade(string CD_FUNDACAO, string NUM_INSCRICAO, DateTime DATA_ATUAL)
		{
			try
			{
				if (AppSettings.IS_SQL_SERVER_PROVIDER)
					return Conexao.Query<DependenteEntidade>("SELECT CS_DEPENDENTE.*,      TB_GRAU_PARENTESCO.DS_GRAU_PARENTESCO,      CS_MOT_PERDA_VALIDADE.DS_MOT_PERDA_VALIDADE,      CS_FUNCIONARIO.NUM_MATRICULA,      CS_FUNCIONARIO.CD_EMPRESA,      EMPRESA.NOME_ENTID AS DS_EMPRESA   FROM CS_DEPENDENTE  INNER JOIN TB_GRAU_PARENTESCO ON TB_GRAU_PARENTESCO.CD_GRAU_PARENTESCO = CS_DEPENDENTE.CD_GRAU_PARENTESCO  LEFT OUTER JOIN CS_MOT_PERDA_VALIDADE ON CS_MOT_PERDA_VALIDADE.CD_MOT_PERDA_VALIDADE = CS_DEPENDENTE.CD_MOT_PERDA_VALIDADE  INNER JOIN CS_FUNCIONARIO ON CS_FUNCIONARIO.NUM_INSCRICAO = CS_DEPENDENTE.NUM_INSCRICAO  INNER JOIN TB_EMPRESA ON TB_EMPRESA.CD_EMPRESA = CS_FUNCIONARIO.CD_EMPRESA  INNER JOIN EE_ENTIDADE EMPRESA ON EMPRESA.COD_ENTID = TB_EMPRESA.COD_ENTID  WHERE CS_DEPENDENTE.CD_FUNDACAO = @CD_FUNDACAO    AND CS_DEPENDENTE.NUM_INSCRICAO = @NUM_INSCRICAO    AND CS_DEPENDENTE.DT_VALIDADE_DEP >= @DATA_ATUAL  ORDER BY CS_DEPENDENTE.NOME_DEP", new { CD_FUNDACAO, NUM_INSCRICAO, DATA_ATUAL }, Transaction).ToList();
				else if (AppSettings.IS_ORACLE_PROVIDER)
					return Conexao.Query<DependenteEntidade>("SELECT CS_DEPENDENTE.*, TB_GRAU_PARENTESCO.DS_GRAU_PARENTESCO, CS_MOT_PERDA_VALIDADE.DS_MOT_PERDA_VALIDADE, CS_FUNCIONARIO.NUM_MATRICULA, CS_FUNCIONARIO.CD_EMPRESA, EMPRESA.NOME_ENTID AS DS_EMPRESA FROM CS_DEPENDENTE INNER  JOIN TB_GRAU_PARENTESCO  ON TB_GRAU_PARENTESCO.CD_GRAU_PARENTESCO=CS_DEPENDENTE.CD_GRAU_PARENTESCO LEFT OUTER JOIN CS_MOT_PERDA_VALIDADE  ON CS_MOT_PERDA_VALIDADE.CD_MOT_PERDA_VALIDADE=CS_DEPENDENTE.CD_MOT_PERDA_VALIDADE INNER  JOIN CS_FUNCIONARIO  ON CS_FUNCIONARIO.NUM_INSCRICAO=CS_DEPENDENTE.NUM_INSCRICAO INNER  JOIN TB_EMPRESA  ON TB_EMPRESA.CD_EMPRESA=CS_FUNCIONARIO.CD_EMPRESA INNER  JOIN EE_ENTIDADE   EMPRESA  ON EMPRESA.COD_ENTID=TB_EMPRESA.COD_ENTID WHERE CS_DEPENDENTE.CD_FUNDACAO=:CD_FUNDACAO AND CS_DEPENDENTE.NUM_INSCRICAO=:NUM_INSCRICAO AND CS_DEPENDENTE.DT_VALIDADE_DEP>=:DATA_ATUAL ORDER BY CS_DEPENDENTE.NOME_DEP", new { CD_FUNDACAO, NUM_INSCRICAO, DATA_ATUAL }, Transaction).ToList();
				else
					throw new Exception("Provider não suportado!");
		...
		public virtual List<DependenteEntidade> BuscarPorFundacaoInscricaoGrupoFamiliarOrderByDataValidade(string CD_FUNDACAO, string NUM_INSCRICAO, decimal NUM_SEQ_GR_FAMIL)
		{
			try
			{
				if (AppSettings.IS_SQL_SERVER_PROVIDER)
					return Conexao.Query<DependenteEntidade>("SELECT DEP.CD_FUNDACAO,         DEP.NUM_INSCRICAO,         DEP.NUM_SEQ_DEP,         GP.DS_GRAU_PARENTESCO,         GP.CD_GRAU_PARENTESCO,         DEP.NOME_DEP,         DEP.DT_NASC_DEP,         DEP.SEXO_DEP,         DEP.CPF,         DEP.PERC_PECULIO,  	   DEP.CD_PLANO,  	   BP.DT_TERMINO_VALIDADE,  	   BP.NUM_SEQ_GR_FAMIL  FROM GB_BENEFICIARIO_PREVIDENCIAL BP    INNER JOIN CS_DEPENDENTE DEP ON DEP.CD_FUNDACAO = BP.CD_FUNDACAO  	AND DEP.NUM_INSCRICAO = BP.NUM_INSCRICAO  	AND DEP.NUM_SEQ_DEP = BP.NUM_SEQ_DEP    INNER JOIN TB_GRAU_PARENTESCO GP ON GP.CD_GRAU_PARENTESCO = DEP.CD_GRAU_PARENTESCO  WHERE BP.CD_FUNDACAO = @CD_FUNDACAO    AND BP.NUM_INSCRICAO = @NUM_INSCRICAO    AND BP.NUM_SEQ_GR_FAMIL = @NUM_SEQ_GR_FAMIL  ORDER BY GP.TIPO_VALIDADE DESC", new { CD_FUNDACAO, NUM_INSCRICAO, NUM_SEQ_GR_FAMIL }, Transaction).ToList();
				else if (AppSettings.IS_ORACLE_PROVIDER)
					return Conexao.Query<DependenteEntidade>("SELECT DEP.CD_FUNDACAO, DEP.NUM_INSCRICAO, DEP.NUM_SEQ_DEP, GP.DS_GRAU_PARENTESCO, GP.CD_GRAU_PARENTESCO, DEP.NOME_DEP, DEP.DT_NASC_DEP, DEP.SEXO_DEP, DEP.CPF, DEP.PERC_PECULIO, DEP.CD_PLANO, BP.DT_TERMINO_VALIDADE, BP.NUM_SEQ_GR_FAMIL FROM GB_BENEFICIARIO_PREVIDENCIAL  BP  INNER  JOIN CS_DEPENDENTE   DEP  ON DEP.CD_FUNDACAO=BP.CD_FUNDACAO AND DEP.NUM_INSCRICAO=BP.NUM_INSCRICAO AND DEP.NUM_SEQ_DEP=BP.NUM_SEQ_DEP INNER  JOIN TB_GRAU_PARENTESCO   GP  ON GP.CD_GRAU_PARENTESCO=DEP.CD_GRAU_PARENTESCO WHERE BP.CD_FUNDACAO=:CD_FUNDACAO AND BP.NUM_INSCRICAO=:NUM_INSCRICAO AND BP.NUM_SEQ_GR_FAMIL=:NUM_SEQ_GR_FAMIL ORDER BY GP.TIPO_VALIDADE DESC", new { CD_FUNDACAO, NUM_INSCRICAO, NUM_SEQ_GR_FAMIL }, Transaction).ToList();
				else
					throw new Exception("Provider não suportado!");
		...
		public virtual List<DependenteEntidade> BuscarPorFundacaoInscricaoPeculio(string CD_FUNDACAO, string NUM_INSCRICAO, string PECULIO)
		{
			try
			{
				if (AppSettings.IS_SQL_SERVER_PROVIDER)
					return Conexao.Query<DependenteEntidade>("SELECT DEP.CD_FUNDACAO,         DEP.NUM_INSCRICAO,         DEP.NUM_SEQ_DEP,         GP.DS_GRAU_PARENTESCO,                DEP.NOME_DEP,         DEP.DT_NASC_DEP,         DEP.SEXO_DEP,                DEP.CPF,         DEP.PERC_PECULIO  FROM CS_DEPENDENTE DEP    INNER JOIN TB_GRAU_PARENTESCO GP ON GP.CD_GRAU_PARENTESCO = DEP.CD_GRAU_PARENTESCO  WHERE DEP.CD_FUNDACAO = @CD_FUNDACAO    AND DEP.NUM_INSCRICAO = @NUM_INSCRICAO    AND DEP.PECULIO = @PECULIO   ORDER BY DEP.NOME_DEP", new { CD_FUNDACAO, NUM_INSCRICAO, PECULIO }, Transaction).ToList();
				else if (AppSettings.IS_ORACLE_PROVIDER)
					return Conexao.Query<DependenteEntidade>("SELECT DEP.CD_FUNDACAO, DEP.NUM_INSCRICAO, DEP.NUM_SEQ_DEP, GP.DS_GRAU_PARENTESCO, DEP.NOME_DEP, DEP.DT_NASC_DEP, DEP.SEXO_DEP, DEP.CPF, DEP.PERC_PECULIO FROM CS_DEPENDENTE  DEP  INNER  JOIN TB_GRAU_PARENTESCO   GP  ON GP.CD_GRAU_PARENTESCO=DEP.CD_GRAU_PARENTESCO WHERE DEP.CD_FUNDACAO=:CD_FUNDACAO AND DEP.NUM_INSCRICAO=:NUM_INSCRICAO AND DEP.PECULIO=:PECULIO ORDER BY DEP.NOME_DEP", new { CD_FUNDACAO, NUM_INSCRICAO, PECULIO }, Transaction).ToList();
				else
					throw new Exception("Provider não suportado!");
		...
		public virtual List<DependenteEntidade> BuscarPorFundacaoInscricaoPlano(string CD_FUNDACAO, string NUM_INSCRICAO, string CD_PLANO)
		{
			...
			{
				if (AppSettings.IS_SQL_SERVER_PROVIDER)
					return Conexao.Query<DependenteEntidade>("SELECT CS_DEPENDENTE.*,  TB_GRAU_PARENTESCO.DS_GRAU_PARENTESCO    FROM CS_DEPENDENTE  INNER JOIN TB_GRAU_PARENTESCO ON TB_GRAU_PARENTESCO.CD_GRAU_PARENTESCO = CS_DEPENDENTE.CD_GRAU_PARENTESCO  WHERE CD_FUNDACAO = @CD_FUNDACAO    AND NUM_INSCRICAO = @NUM_INSCRICAO    AND CD_PLANO = @CD_PLANO  ORDER BY NOME_DEP", new { CD_FUNDACAO, NUM_INSCRICAO, CD_PLANO }, Transaction).ToList();
				else if (AppSettings.IS_ORACLE_PROVIDER)
					return Conexao.Query<DependenteEntidade>("SELECT CS_DEPENDENTE.*, TB_GRAU_PARENTESCO.DS_GRAU_PARENTESCO FROM CS_DEPENDENTE INNER  JOIN TB_GRAU_PARENTESCO  ON TB_GRAU_PARENTESCO.CD_GRAU_PARENTESCO=CS_DEPENDENTE.CD_GRAU_PARENTESCO WHERE CD_FUNDACAO=:CD_FUNDACAO AND NUM_INSCRICAO=:NUM_INSCRICAO AND CD_PLANO=:CD_PLANO ORDER BY NOME_DEP", new { CD_FUNDACAO, NUM_INSCRICAO, CD_PLANO }, Transaction).ToList();
				else
					throw new Exception("Provider não suportado!");
		...
		public virtual List<DependenteEntidade> BuscarPorFundacaoInscricaoPlanoDataAtualPeculio(string CD_FUNDACAO, string NUM_INSCRICAO, string CD_PLANO, DateTime DT_ATUAL, string PECULIO)
		{
			...
			{
				if (AppSettings.IS_SQL_SERVER_PROVIDER)
					return Conexao.Query<DependenteEntidade>("SELECT *   FROM CS_DEPENDENTE  INNER JOIN TB_GRAU_PARENTESCO ON TB_GRAU_PARENTESCO.CD_GRAU_PARENTESCO = CS_DEPENDENTE.CD_GRAU_PARENTESCO  WHERE CD_FUNDACAO        = @CD_FUNDACAO    AND NUM_INSCRICAO      = @NUM_INSCRICAO    AND CD_PLANO           = @CD_PLANO    AND DT_VALIDADE_DEP    >= @DT_ATUAL    AND PECULIO            = @PECULIO", new { CD_FUNDACAO, NUM_INSCRICAO, CD_PLANO, DT_ATUAL, PECULIO }, Transaction).ToList();
				else if (AppSettings.IS_ORACLE_PROVIDER)
					return Conexao.Query<DependenteEntidade>("SELECT * FROM CS_DEPENDENTE INNER  JOIN TB_GRAU_PARENTESCO  ON TB_GRAU_PARENTESCO.CD_GRAU_PARENTESCO=CS_DEPENDENTE.CD_GRAU_PARENTESCO WHERE CD_FUNDACAO=:CD_FUNDACAO AND NUM_INSCRICAO=:NUM_INSCRICAO AND CD_PLANO=:CD_PLANO AND DT_VALIDADE_DEP>=:DT_ATUAL AND PECULIO=:PECULIO", new { CD_FUNDACAO, NUM_INSCRICAO, CD_PLANO, DT_ATUAL, PECULIO }, Transaction).ToList();
				else
					throw new Exception("Provider não suportado!");
		...
		public virtual List<DependenteEntidade> BuscarPorFundacaoInscricaoPlanoDataAtualPlanoPrevidencial(string CD_FUNDACAO, string NUM_INSCRICAO, string CD_PLANO, DateTime DT_ATUAL, string PLANO_PREVIDENCIAL)
		{
			try
			{
				if (AppSettings.IS_SQL_SERVER_PROVIDER)
					return Conexao.Query<DependenteEntidade>("SELECT *   FROM CS_DEPENDENTE  INNER JOIN TB_GRAU_PARENTESCO ON TB_GRAU_PARENTESCO.CD_GRAU_PARENTESCO = CS_DEPENDENTE.CD_GRAU_PARENTESCO  WHERE CD_FUNDACAO        = @CD_FUNDACAO    AND NUM_INSCRICAO      = @NUM_INSCRICAO    AND CD_PLANO           = @CD_PLANO    AND DT_VALIDADE_DEP    >= @DT_ATUAL    AND PLANO_PREVIDENCIAL = @PLANO_PREVIDENCIAL", new { CD_FUNDACAO, NUM_INSCRICAO, CD_PLANO, DT_ATUAL, PLANO_PREVIDENCIAL }, Transaction).ToList();
				else if (AppSettings.IS_ORACLE_PROVIDER)
					return Conexao.Query<DependenteEntidade>("SELECT * FROM CS_DEPENDENTE INNER  JOIN TB_GRAU_PARENTESCO  ON TB_GRAU_PARENTESCO.CD_GRAU_PARENTESCO=CS_DEPENDENTE.CD_GRAU_PARENTESCO WHERE CD_FUNDACAO=:CD_FUNDACAO AND NUM_INSCRICAO=:NUM_INSCRICAO AND CD_PLANO=:CD_PLANO AND DT_VALIDADE_DEP>=:DT_ATUAL AND PLANO_PREVIDENCIAL=:PLANO_PREVIDENCIAL", new { CD_FUNDACAO, NUM_INSCRICAO, CD_PLANO, DT_ATUAL, PLANO_PREVIDENCIAL }, Transaction).ToList();
				else
					throw new Exception("Provider não suportado!");
		...
		public virtual List<DependenteEntidade> BuscarPorFundacaoInscricaoPlanoPlanoPrevidencialFixoOrderByGrauValidade(string CD_FUNDACAO, string NUM_INSCRICAO, string CD_PLANO)
		{
			...
			{
				if (AppSettings.IS_SQL_SERVER_PROVIDER)
					return Conexao.Query<DependenteEntidade>("SELECT DEP.CD_FUNDACAO,         DEP.NUM_INSCRICAO,         DEP.NUM_SEQ_DEP,  	   GP.CD_GRAU_PARENTESCO,         GP.DS_GRAU_PARENTESCO,                DEP.NOME_DEP,         DEP.DT_NASC_DEP,         DEP.SEXO_DEP,                DEP.CPF,         DEP.PERC_PECULIO,  	   TIPO_HERDEIRO,  	   CD_PLANO  FROM CS_DEPENDENTE DEP    INNER JOIN TB_GRAU_PARENTESCO GP ON GP.CD_GRAU_PARENTESCO = DEP.CD_GRAU_PARENTESCO  WHERE DEP.CD_FUNDACAO = @CD_FUNDACAO    AND NUM_INSCRICAO = @NUM_INSCRICAO    AND CD_PLANO = @CD_PLANO    AND PLANO_PREVIDENCIAL = 'S'  ORDER BY GP.TIPO_VALIDADE DESC", new { CD_FUNDACAO, NUM_INSCRICAO, CD_PLANO }, Transaction).ToList();
				else if (AppSettings.IS_ORACLE_PROVIDER)
					return Conexao.Query<DependenteEntidade>("SELECT DEP.CD_FUNDACAO, DEP.NUM_INSCRICAO, DEP.NUM_SEQ_DEP, GP.CD_GRAU_PARENTESCO, GP.DS_GRAU_PARENTESCO, DEP.NOME_DEP, DEP.DT_NASC_DEP, DEP.SEXO_DEP, DEP.CPF, DEP.PERC_PECULIO, TIPO_HERDEIRO, CD_PLANO FROM CS_DEPENDENTE  DEP  INNER  JOIN TB_GRAU_PARENTESCO   GP  ON GP.CD_GRAU_PARENTESCO=DEP.CD_GRAU_PARENTESCO WHERE DEP.CD_FUNDACAO=:CD_FUNDACAO AND NUM_INSCRICAO=:NUM_INSCRICAO AND CD_PLANO=:CD_PLANO AND PLANO_PREVIDENCIAL='S' ORDER BY GP.TIPO_VALIDADE DESC", new { CD_FUNDACAO, NUM_INSCRICAO, CD_PLANO }, Transaction).ToList();
				else
					throw new Exception("Provider não suportado!");
		...
		public virtual List<DependenteEntidade> BuscarPorFundacaoInscricaoPlanoPrevidencialFixoOrderByGrauValidade(string CD_FUNDACAO, string NUM_INSCRICAO)
		{
			try
			{
				if (AppSettings.IS_SQL_SERVER_PROVIDER)
					return Conexao.Query<DependenteEntidade>("SELECT DEP.CD_FUNDACAO,         DEP.NUM_INSCRICAO,         DEP.NUM_SEQ_DEP,         GP.DS_GRAU_PARENTESCO,         GP.CD_GRAU_PARENTESCO,         DEP.NOME_DEP,         DEP.DT_NASC_DEP,         DEP.SEXO_DEP,         DEP.CPF,         DEP.PERC_PECULIO,  	   DEP.DT_INCLUSAO_DEP,  	   DEP.CD_PLANO,  	   DEP.DT_VALIDADE_DEP  FROM CS_DEPENDENTE DEP    INNER JOIN TB_GRAU_PARENTESCO GP ON GP.CD_GRAU_PARENTESCO = DEP.CD_GRAU_PARENTESCO  WHERE DEP.CD_FUNDACAO = @CD_FUNDACAO    AND NUM_INSCRICAO = @NUM_INSCRICAO    AND PLANO_PREVIDENCIAL = 'S'  ORDER BY GP.TIPO_VALIDADE DESC", new { CD_FUNDACAO, NUM_INSCRICAO }, Transaction).ToList();
				else if (AppSettings.IS_ORACLE_PROVIDER)
					return Conexao.Query<DependenteEntidade>("SELECT DEP.CD_FUNDACAO, DEP.NUM_INSCRICAO, DEP.NUM_SEQ_DEP, GP.DS_GRAU_PARENTESCO, GP.CD_GRAU_PARENTESCO, DEP.NOME_DEP, DEP.DT_NASC_DEP, DEP.SEXO_DEP, DEP.CPF, DEP.PERC_PECULIO, DEP.DT_INCLUSAO_DEP, DEP.CD_PLANO, DEP.DT_VALIDADE_DEP FROM CS_DEPENDENTE  DEP  INNER  JOIN TB_GRAU_PARENTESCO   GP  ON GP.CD_GRAU_PARENTESCO=DEP.CD_GRAU_PARENTESCO WHERE DEP.CD_FUNDACAO=:CD_FUNDACAO AND NUM_INSCRICAO=:NUM_INSCRICAO AND PLANO_PREVIDENCIAL='S' ORDER BY GP.TIPO_VALIDADE DESC", new { CD_FUNDACAO, NUM_INSCRICAO }, Transaction).ToList();
				else
					throw new Exception("Provider não suportado!");
		...
		public virtual List<DependenteEntidade> BuscarPorPlanoFundacaoInscricao(string CD_FUNDACAO, string NUM_INSCRICAO, string CD_PLANO)
		{
			try
			{
				if (AppSettings.IS_SQL_SERVER_PROVIDER)
					return Conexao.Query<DependenteEntidade>("SELECT CS_DEPENDENTE.*,      TB_GRAU_PARENTESCO.DS_GRAU_PARENTESCO,      CS_MOT_PERDA_VALIDADE.DS_MOT_PERDA_VALIDADE,      CS_FUNCIONARIO.NUM_MATRICULA,      CS_FUNCIONARIO.CD_EMPRESA,      EMPRESA.NOME_ENTID AS DS_EMPRESA   FROM CS_DEPENDENTE  INNER JOIN TB_GRAU_PARENTESCO ON TB_GRAU_PARENTESCO.CD_GRAU_PARENTESCO = CS_DEPENDENTE.CD_GRAU_PARENTESCO  LEFT OUTER JOIN CS_MOT_PERDA_VALIDADE ON CS_MOT_PERDA_VALIDADE.CD_MOT_PERDA_VALIDADE = CS_DEPENDENTE.CD_MOT_PERDA_VALIDADE  INNER JOIN CS_FUNCIONARIO ON CS_FUNCIONARIO.NUM_INSCRICAO = CS_DEPENDENTE.NUM_INSCRICAO  INNER JOIN TB_EMPRESA ON TB_EMPRESA.CD_EMPRESA = CS_FUNCIONARIO.CD_EMPRESA  INNER JOIN EE_ENTIDADE EMPRESA ON EMPRESA.COD_ENTID = TB_EMPRESA.COD_ENTID  WHERE CS_DEPENDENTE.CD_FUNDACAO = @CD_FUNDACAO    AND CS_DEPENDENTE.NUM_INSCRICAO = @NUM_INSCRICAO    AND CS_DEPENDENTE.CD_PLANO = @CD_PLANO  ORDER BY CS_DEPENDENTE.NOME_DEP", new { CD_FUNDACAO, NUM_INSCRICAO, CD_PLANO }, Transaction).ToList();
				else if (AppSettings.IS_ORACLE_PROVIDER)
					return Conexao.Query<DependenteEntidade>("SELECT CS_DEPENDENTE.*, TB_GRAU_PARENTESCO.DS_GRAU_PARENTESCO, CS_MOT_PERDA_VALIDADE.DS_MOT_PERDA_VALIDADE, CS_FUNCIONARIO.NUM_MATRICULA, CS_FUNCIONARIO.CD_EMPRESA, EMPRESA.NOME_ENTID AS DS_EMPRESA FROM CS_DEPENDENTE INNER  JOIN TB_GRAU_PARENTESCO  ON TB_GRAU_PARENTESCO.CD_GRAU_PARENTESCO=CS_DEPENDENTE.CD_GRAU_PARENTESCO LEFT OUTER JOIN CS_MOT_PERDA_VALIDADE  ON CS_MOT_PERDA_VALIDADE.CD_MOT_PERDA_VALIDADE=CS_DEPENDENTE.CD_MOT_PERDA_VALIDADE INNER  JOIN CS_FUNCIONARIO  ON CS_FUNCIONARIO.NUM_INSCRICAO=CS_DEPENDENTE.NUM_INSCRICAO INNER  JOIN TB_EMPRESA  ON TB_EMPRESA.CD_EMPRESA=CS_FUNCIONARIO.CD_EMPRESA INNER  JOIN EE_ENTIDADE   EMPRESA  ON EMPRESA.COD_ENTID=TB_EMPRESA.COD_ENTID WHERE CS_DEPENDENTE.CD_FUNDACAO=:CD_FUNDACAO AND CS_DEPENDENTE.NUM_INSCRICAO=:NUM_INSCRICAO AND CS_DEPENDENTE.CD_PLANO=:CD_PLANO ORDER BY CS_DEPENDENTE.NOME_DEP", new { CD_FUNDACAO, NUM_INSCRICAO, CD_PLANO }, Transaction).ToList();
				else
					throw new Exception("Provider não suportado!");
		...
		public virtual List<DependenteEntidade> BuscarPorPlanoFundacaoInscricaoDataAtual(string CD_FUNDACAO, string NUM_INSCRICAO, string CD_PLANO, DateTime DATA_ATUAL)
		{
			try
			{
				if (AppSettings.IS_SQL_SERVER_PROVIDER)
					return Conexao.Query<DependenteEntidade>("SELECT CS_DEPENDENTE.*,      TB_GRAU_PARENTESCO.DS_GRAU_PARENTESCO,      CS_MOT_PERDA_VALIDADE.DS_MOT_PERDA_VALIDADE,      CS_FUNCIONARIO.NUM_MATRICULA,      CS_FUNCIONARIO.CD_EMPRESA,      EMPRESA.NOME_ENTID AS DS_EMPRESA   FROM CS_DEPENDENTE  INNER JOIN TB_GRAU_PARENTESCO ON TB_GRAU_PARENTESCO.CD_GRAU_PARENTESCO = CS_DEPENDENTE.CD_GRAU_PARENTESCO  LEFT OUTER JOIN CS_MOT_PERDA_VALIDADE ON CS_MOT_PERDA_VALIDADE.CD_MOT_PERDA_VALIDADE = CS_DEPENDENTE.CD_MOT_PERDA_VALIDADE  INNER JOIN CS_FUNCIONARIO ON CS_FUNCIONARIO.NUM_INSCRICAO = CS_DEPENDENTE.NUM_INSCRICAO  INNER JOIN TB_EMPRESA ON TB_EMPRESA.CD_EMPRESA = CS_FUNCIONARIO.CD_EMPRESA  INNER JOIN EE_ENTIDADE EMPRESA ON EMPRESA.COD_ENTID = TB_EMPRESA.COD_ENTID  WHERE CS_DEPENDENTE.CD_FUNDACAO = @CD_FUNDACAO    AND CS_DEPENDENTE.NUM_INSCRICAO = @NUM_INSCRICAO    AND CS_DEPENDENTE.CD_PLANO = @CD_PLANO    AND CS_DEPENDENTE.DT_VALIDADE_DEP >= @DATA_ATUAL  ORDER BY CS_DEPENDENTE.NOME_DEP", new { CD_FUNDACAO, NUM_INSCRICAO, CD_PLANO, DATA_ATUAL }, Transaction).ToList();
				else if (AppSettings.IS_ORACLE_PROVIDER)
					return Conexao.Query<DependenteEntidade>("SELECT CS_DEPENDENTE.*, TB_GRAU_PARENTESCO.DS_GRAU_PARENTESCO, CS_MOT_PERDA_VALIDADE.DS_MOT_PERDA_VALIDADE, CS_FUNCIONARIO.NUM_MATRICULA, CS_FUNCIONARIO.CD_EMPRESA, EMPRESA.NOME_ENTID AS DS_EMPRESA FROM CS_DEPENDENTE INNER  JOIN TB_GRAU_PARENTESCO  ON TB_GRAU_PARENTESCO.CD_GRAU_PARENTESCO=CS_DEPENDENTE.CD_GRAU_PARENTESCO LEFT OUTER JOIN CS_MOT_PERDA_VALIDADE  ON CS_MOT_PERDA_VALIDADE.CD_MOT_PERDA_VALIDADE=CS_DEPENDENTE.CD_MOT_PERDA_VALIDADE INNER  JOIN CS_FUNCIONARIO  ON CS_FUNCIONARIO.NUM_INSCRICAO=CS_DEPENDENTE.NUM_INSCRICAO INNER  JOIN TB_EMPRESA  ON TB_EMPRESA.CD_EMPRESA=CS_FUNCIONARIO.CD_EMPRESA INNER  JOIN EE_ENTIDADE   EMPRESA  ON EMPRESA.COD_ENTID=TB_EMPRESA.COD_ENTID WHERE CS_DEPENDENTE.CD_FUNDACAO=:CD_FUNDACAO AND CS_DEPENDENTE.NUM_INSCRICAO=:NUM_INSCRICAO AND CS_DEPENDENTE.CD_PLANO=:CD_PLANO AND CS_DEPENDENTE.DT_VALIDADE_DEP>=:DATA_ATUAL ORDER BY CS_DEPENDENTE.NOME_DEP", new { CD_FUNDACAO, NUM_INSCRICAO, CD_PLANO, DATA_ATUAL }, Transaction).ToList();
				else
					throw new Exception("Provider não suportado!");
		...
		public virtual void ExcluirDependente(string CD_FUNDACAO, string NUM_INSCRICAO, decimal NUM_SEQ_DEP)
		{
			...
			{
				if (AppSettings.IS_SQL_SERVER_PROVIDER)
					Conexao.Execute("DELETE FROM CS_DEPENDENTE   WHERE CD_FUNDACAO = @CD_FUNDACAO    AND NUM_INSCRICAO =@NUM_INSCRICAO    AND NUM_SEQ_DEP = @NUM_SEQ_DEP", new { CD_FUNDACAO, NUM_INSCRICAO, NUM_SEQ_DEP }, Transaction);
				else if (AppSettings.IS_ORACLE_PROVIDER)
					Conexao.Execute("DELETE FROM CS_DEPENDENTE WHERE CD_FUNDACAO=:CD_FUNDACAO AND NUM_INSCRICAO=:NUM_INSCRICAO AND NUM_SEQ_DEP=:NUM_SEQ_DEP", new { CD_FUNDACAO, NUM_INSCRICAO, NUM_SEQ_DEP }, Transaction);
				else
					throw new Exception("Provider não suportado!");
		...
		public virtual int ObterUltimoSequencial(string CD_FUNDACAO, string NUM_INSCRICAO)
		{
			...
			{
				if (AppSettings.IS_SQL_SERVER_PROVIDER)
					return Conexao.QuerySingleOrDefault<int>("SELECT MAX(NUM_SEQ_DEP) AS NUM_SEQ_DEP   FROM CS_DEPENDENTE  WHERE CD_FUNDACAO = @CD_FUNDACAO    AND NUM_INSCRICAO = @NUM_INSCRICAO", new { CD_FUNDACAO, NUM_INSCRICAO }, Transaction);
				else if (AppSettings.IS_ORACLE_PROVIDER)
					return Conexao.QuerySingleOrDefault<int>("SELECT MAX(NUM_SEQ_DEP) AS NUM_SEQ_DEP FROM CS_DEPENDENTE WHERE CD_FUNDACAO=:CD_FUNDACAO AND NUM_INSCRICAO=:NUM_INSCRICAO", new { CD_FUNDACAO, NUM_INSCRICAO }, Transaction);
				else
					throw new Exception("Provider não suportado!");
