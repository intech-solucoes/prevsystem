...
namespace Intech.PrevSystem.Dados.DAO
{
	public abstract class DocumentoPastaDAO : BaseDAO<DocumentoPastaEntidade>
	{
		...
		public virtual List<DocumentoPastaEntidade> BuscarPorOidPastaPaiJoinWebGrupoUsuario(decimal? OID_DOCUMENTO_PASTA_PAI)
		{
			try
			{
				if (AppSettings.IS_SQL_SERVER_PROVIDER)
					return Conexao.Query<DocumentoPastaEntidade>("SELECT *  FROM WEB_DOCUMENTO_PASTA WDP LEFT JOIN WEB_GRUPO_USUARIO WGU  ON WDP.OID_GRUPO_USUARIO = WGU.OID_GRUPO_USUARIO  WHERE (OID_DOCUMENTO_PASTA_PAI = @OID_DOCUMENTO_PASTA_PAI)     OR (@OID_DOCUMENTO_PASTA_PAI IS NULL AND OID_DOCUMENTO_PASTA_PAI IS NULL)", new { OID_DOCUMENTO_PASTA_PAI }).ToList();
				else if (AppSettings.IS_ORACLE_PROVIDER)
					return Conexao.Query<DocumentoPastaEntidade>("SELECT * FROM WEB_DOCUMENTO_PASTA  WDP  LEFT JOIN WEB_GRUPO_USUARIO   WGU  ON WDP.OID_GRUPO_USUARIO=WGU.OID_GRUPO_USUARIO WHERE (OID_DOCUMENTO_PASTA_PAI=:OID_DOCUMENTO_PASTA_PAI) OR (:OID_DOCUMENTO_PASTA_PAI IS NULL  AND OID_DOCUMENTO_PASTA_PAI IS NULL )", new { OID_DOCUMENTO_PASTA_PAI }).ToList();
				else
					throw new Exception("Provider não suportado!");
		...
		public virtual List<DocumentoPastaEntidade> BuscarPorWebUsuario(string NOM_LOGIN, decimal? OID_DOCUMENTO_PASTA_PAI)
		{
			try
			{
				if (AppSettings.IS_SQL_SERVER_PROVIDER)
					return Conexao.Query<DocumentoPastaEntidade>("SELECT WDP.OID_DOCUMENTO_PASTA, WDP.OID_DOCUMENTO_PASTA_PAI, WDP.NOM_PASTA, WDP.OID_GRUPO_USUARIO   FROM WEB_DOCUMENTO_PASTA WDP   LEFT JOIN WEB_USUARIO_GRUPO WUG   ON WDP.OID_GRUPO_USUARIO = WUG.OID_GRUPO_USUARIO   LEFT JOIN WEB_USUARIO U ON U.OID_USUARIO = WUG.OID_USUARIO   WHERE (NOM_LOGIN = @NOM_LOGIN OR NOM_LOGIN IS NULL)     AND ((OID_DOCUMENTO_PASTA_PAI = @OID_DOCUMENTO_PASTA_PAI)     OR (@OID_DOCUMENTO_PASTA_PAI IS NULL AND OID_DOCUMENTO_PASTA_PAI IS NULL))", new { NOM_LOGIN, OID_DOCUMENTO_PASTA_PAI }).ToList();
				else if (AppSettings.IS_ORACLE_PROVIDER)
					return Conexao.Query<DocumentoPastaEntidade>("SELECT WDP.OID_DOCUMENTO_PASTA, WDP.OID_DOCUMENTO_PASTA_PAI, WDP.NOM_PASTA, WDP.OID_GRUPO_USUARIO FROM WEB_DOCUMENTO_PASTA  WDP  LEFT JOIN WEB_USUARIO_GRUPO   WUG  ON WDP.OID_GRUPO_USUARIO=WUG.OID_GRUPO_USUARIO LEFT JOIN WEB_USUARIO   U  ON U.OID_USUARIO=WUG.OID_USUARIO WHERE (NOM_LOGIN=:NOM_LOGIN OR NOM_LOGIN IS NULL ) AND ((OID_DOCUMENTO_PASTA_PAI=:OID_DOCUMENTO_PASTA_PAI) OR (:OID_DOCUMENTO_PASTA_PAI IS NULL  AND OID_DOCUMENTO_PASTA_PAI IS NULL ))", new { NOM_LOGIN, OID_DOCUMENTO_PASTA_PAI }).ToList();
				else
					throw new Exception("Provider não suportado!");
