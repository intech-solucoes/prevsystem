...
namespace Intech.PrevSystem.Dados.DAO
{
	public abstract class FuncionarioDAO : BaseDAO<FuncionarioEntidade>
	{
		...
		public virtual List<FuncionarioEntidade> BuscarPorPesquisa(string CD_FUNDACAO, string CD_EMPRESA, string CD_PLANO, string CD_SIT_PLANO, string NUM_MATRICULA, string NOME)
		{
			try
			{
				if (AppSettings.IS_SQL_SERVER_PROVIDER)
					return Conexao.Query<FuncionarioEntidade>("SELECT DISTINCT *  FROM   VW_FUNC_PLANO_DADOS  WHERE (CD_FUNDACAO = @CD_FUNDACAO OR @CD_FUNDACAO IS NULL)    AND (CD_EMPRESA = @CD_EMPRESA OR @CD_EMPRESA IS NULL)    AND (CD_PLANO = @CD_PLANO OR @CD_PLANO IS NULL)    AND (CD_SIT_PLANO = @CD_SIT_PLANO OR @CD_SIT_PLANO IS NULL)    AND (NUM_MATRICULA LIKE '%' + @NUM_MATRICULA + '%' OR @NUM_MATRICULA IS NULL)     AND (NOME_ENTID LIKE '%' + @NOME + '%' OR @NOME IS NULL)", new { CD_FUNDACAO, CD_EMPRESA, CD_PLANO, CD_SIT_PLANO, NUM_MATRICULA, NOME }).ToList();
				else if (AppSettings.IS_ORACLE_PROVIDER)
					return Conexao.Query<FuncionarioEntidade>("SELECT DISTINCT * FROM VW_FUNC_PLANO_DADOS WHERE (CD_FUNDACAO=:CD_FUNDACAO OR :CD_FUNDACAO IS NULL ) AND (CD_EMPRESA=:CD_EMPRESA OR :CD_EMPRESA IS NULL ) AND (CD_PLANO=:CD_PLANO OR :CD_PLANO IS NULL ) AND (CD_SIT_PLANO=:CD_SIT_PLANO OR :CD_SIT_PLANO IS NULL ) AND (NUM_MATRICULA LIKE '%' || :NUM_MATRICULA || '%' OR :NUM_MATRICULA IS NULL ) AND (NOME_ENTID LIKE '%' || :NOME || '%' OR :NOME IS NULL )", new { CD_FUNDACAO, CD_EMPRESA, CD_PLANO, CD_SIT_PLANO, NUM_MATRICULA, NOME }).ToList();
				else
					throw new Exception("Provider não suportado!");
		...
		public virtual FuncionarioEntidade Buscarwtf(string COD_ENTID)
		{
			try
			{
				if (AppSettings.IS_SQL_SERVER_PROVIDER)
					return Conexao.QuerySingleOrDefault<FuncionarioEntidade>("SELECT  	EE_ENTIDADE.NOME_ENTID,       	TB_LOTACAO.DS_LOTACAO,  	TB_CARGO.DS_CARGO,  	EE-ENTIDADE.CPF_CGC,  	CS_FUNCIONARIO.*   FROM CS_FUNCIONARIO   INNER JOIN EE_ENTIDADE ON EE_ENTIDADE.COD_ENTID = CS_FUNCIONARIO.COD_ENTID   LEFT JOIN TB_LOTACAO ON TB_LOTACAO.CD_LOTACAO = CS_FUNCIONARIO.CD_LOTACAO                      AND TB_LOTACAO.CD_EMPRESA = CS_FUNCIONARIO.CD_EMPRESA  LEFT JOIN TB_CARGO ON TB_CARGO.CD_CARGO = CS_FUNCIONARIO.CD_CARGO                    AND TB_CARGO.CD_EMPRESA = CS_FUNCIONARIO.CD_EMPRESA  WHERE CS_FUNCIONARIO.COD_ENTID = @COD_ENTID  ORDER BY CS_FUNCIONARIO.DT_ADMISSAO DESC", new { COD_ENTID });
				else if (AppSettings.IS_ORACLE_PROVIDER)
					return Conexao.QuerySingleOrDefault<FuncionarioEntidade>("SELECT EE_ENTIDADE.NOME_ENTID, TB_LOTACAO.DS_LOTACAO, TB_CARGO.DS_CARGO, EE - ENTIDADE.CPF_CGC, CS_FUNCIONARIO.* FROM CS_FUNCIONARIO INNER  JOIN EE_ENTIDADE  ON EE_ENTIDADE.COD_ENTID=CS_FUNCIONARIO.COD_ENTID LEFT JOIN TB_LOTACAO  ON TB_LOTACAO.CD_LOTACAO=CS_FUNCIONARIO.CD_LOTACAO AND TB_LOTACAO.CD_EMPRESA=CS_FUNCIONARIO.CD_EMPRESA LEFT JOIN TB_CARGO  ON TB_CARGO.CD_CARGO=CS_FUNCIONARIO.CD_CARGO AND TB_CARGO.CD_EMPRESA=CS_FUNCIONARIO.CD_EMPRESA WHERE CS_FUNCIONARIO.COD_ENTID=:COD_ENTID ORDER BY CS_FUNCIONARIO.DT_ADMISSAO DESC", new { COD_ENTID });
				else
					throw new Exception("Provider não suportado!");
