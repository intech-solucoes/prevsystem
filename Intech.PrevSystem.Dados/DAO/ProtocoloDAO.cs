...
namespace Intech.PrevSystem.Dados.DAO
{
	public abstract class ProtocoloDAO : BaseDAO<ProtocoloEntidade>
	{
		...
		public virtual List<ProtocoloEntidade> BuscarPorFundacaoEmpresaMatricula(string CD_FUNDACAO, string CD_EMPRESA, string NUM_MATRICULA)
		{
			...
			{
				if (AppSettings.IS_SQL_SERVER_PROVIDER)
					return Conexao.Query<ProtocoloEntidade>("SELECT WEB_PROTOCOLO.*,     WEB_FUNCIONALIDADE.DES_FUNCIONALIDADE,     EE_ENTIDADE.NOME_ENTID,     TB_PLANOS.DS_PLANO FROM   WEB_PROTOCOLO  INNER JOIN WEB_FUNCIONALIDADE ON WEB_FUNCIONALIDADE.OID_FUNCIONALIDADE = WEB_PROTOCOLO.OID_FUNCIONALIDADE  INNER JOIN CS_FUNCIONARIO ON CS_FUNCIONARIO.NUM_MATRICULA = WEB_PROTOCOLO.NUM_MATRICULA INNER JOIN EE_ENTIDADE ON EE_ENTIDADE.COD_ENTID = CS_FUNCIONARIO.COD_ENTID INNER JOIN WEB_FUNCIONALIDADE ON WEB_FUNCIONALIDADE.OID_FUNCIONALIDADE = WEB_PROTOCOLO.OID_FUNCIONALIDADE  LEFT OUTER JOIN TB_PLANOS ON TB_PLANOS.CD_PLANO = WEB_PROTOCOLO.CD_PLANO WHERE ( WEB_PROTOCOLO.CD_FUNDACAO = @CD_FUNDACAO )    AND ( WEB_PROTOCOLO.CD_EMPRESA = @CD_EMPRESA )    AND ( WEB_PROTOCOLO.NUM_MATRICULA = @NUM_MATRICULA )  ORDER  BY WEB_PROTOCOLO.DTA_SOLICITACAO DESC", new { CD_FUNDACAO, CD_EMPRESA, NUM_MATRICULA }, Transaction).ToList();
				else if (AppSettings.IS_ORACLE_PROVIDER)
					return Conexao.Query<ProtocoloEntidade>("SELECT WEB_PROTOCOLO.*, WEB_FUNCIONALIDADE.DES_FUNCIONALIDADE, EE_ENTIDADE.NOME_ENTID, TB_PLANOS.DS_PLANO FROM WEB_PROTOCOLO INNER  JOIN WEB_FUNCIONALIDADE  ON WEB_FUNCIONALIDADE.OID_FUNCIONALIDADE=WEB_PROTOCOLO.OID_FUNCIONALIDADE INNER  JOIN CS_FUNCIONARIO  ON CS_FUNCIONARIO.NUM_MATRICULA=WEB_PROTOCOLO.NUM_MATRICULA INNER  JOIN EE_ENTIDADE  ON EE_ENTIDADE.COD_ENTID=CS_FUNCIONARIO.COD_ENTID INNER  JOIN WEB_FUNCIONALIDADE  ON WEB_FUNCIONALIDADE.OID_FUNCIONALIDADE=WEB_PROTOCOLO.OID_FUNCIONALIDADE LEFT OUTER JOIN TB_PLANOS  ON TB_PLANOS.CD_PLANO=WEB_PROTOCOLO.CD_PLANO WHERE (WEB_PROTOCOLO.CD_FUNDACAO=:CD_FUNDACAO) AND (WEB_PROTOCOLO.CD_EMPRESA=:CD_EMPRESA) AND (WEB_PROTOCOLO.NUM_MATRICULA=:NUM_MATRICULA) ORDER BY WEB_PROTOCOLO.DTA_SOLICITACAO DESC", new { CD_FUNDACAO, CD_EMPRESA, NUM_MATRICULA }, Transaction).ToList();
				else
					throw new Exception("Provider não suportado!");
