...
{   
    public abstract class FuncionarioDAO : BaseDAO<FuncionarioEntidade>
    {
		...
		public virtual FuncionarioEntidade BuscarPorCodEntid(string COD_ENTID)
		{
			try
			{
				if(AppSettings.IS_SQL_SERVER_PROVIDER)
					return Conexao.QuerySingleOrDefault<FuncionarioEntidade>("SELECT       	EE_ENTIDADE.NOME_ENTID,      	TB_LOTACAO.DS_LOTACAO, 	TB_CARGO.DS_CARGO, 	CS_FUNCIONARIO.*  FROM CS_FUNCIONARIO  INNER JOIN EE_ENTIDADE ON EE_ENTIDADE.COD_ENTID = CS_FUNCIONARIO.COD_ENTID  INNER JOIN TB_LOTACAO ON TB_LOTACAO.CD_LOTACAO = CS_FUNCIONARIO.CD_LOTACAO                      AND TB_LOTACAO.CD_EMPRESA = CS_FUNCIONARIO.CD_EMPRESA INNER JOIN TB_CARGO ON TB_CARGO.CD_CARGO = CS_FUNCIONARIO.CD_CARGO                    AND TB_CARGO.CD_EMPRESA = CS_FUNCIONARIO.CD_EMPRESA WHERE CS_FUNCIONARIO.COD_ENTID = @COD_ENTID", new { COD_ENTID });
		...
		public virtual IEnumerable<FuncionarioEntidade> BuscarPorPesquisa(string CD_FUNDACAO, string CD_EMPRESA, string CD_PLANO, string CD_SIT_PLANO, string NUM_MATRICULA)
		{
			try
			{
				if(AppSettings.IS_SQL_SERVER_PROVIDER)
					return Conexao.Query<FuncionarioEntidade>("SELECT * FROM   VW_FUNC_PLANO_DADOS WHERE (CD_FUNDACAO = @CD_FUNDACAO OR @CD_FUNDACAO IS NULL)   AND (CD_EMPRESA = @CD_EMPRESA OR @CD_EMPRESA IS NULL)   AND (CD_PLANO = @CD_PLANO OR @CD_PLANO IS NULL)   AND (CD_SIT_PLANO = @CD_SIT_PLANO OR @CD_SIT_PLANO IS NULL)   AND (NUM_MATRICULA LIKE '%' + @NUM_MATRICULA + '%' OR @NUM_MATRICULA IS NULL)", new { CD_FUNDACAO, CD_EMPRESA, CD_PLANO, CD_SIT_PLANO, NUM_MATRICULA });
				else if(AppSettings.IS_ORACLE_PROVIDER)
					return Conexao.Query<FuncionarioEntidade>("SELECT * FROM VW_FUNC_PLANO_DADOS WHERE (CD_FUNDACAO=:CD_FUNDACAO OR :CD_FUNDACAO IS NULL ) AND (CD_EMPRESA=:CD_EMPRESA OR :CD_EMPRESA IS NULL ) AND (CD_PLANO=:CD_PLANO OR :CD_PLANO IS NULL ) AND (CD_SIT_PLANO=:CD_SIT_PLANO OR :CD_SIT_PLANO IS NULL ) AND (NUM_MATRICULA LIKE '%' || :NUM_MATRICULA || '%' OR :NUM_MATRICULA IS NULL )", new { CD_FUNDACAO, CD_EMPRESA, CD_PLANO, CD_SIT_PLANO, NUM_MATRICULA });
				else
					throw new Exception("Provider não suportado!");
