...
namespace Intech.PrevSystem.Dados.DAO
{
	...
	{
		public DocumentoDAO (IDbTransaction tx = null) : base(tx) { }
		...
		public virtual DocumentoEntidade BuscarPorOid(decimal OID_DOCUMENTO)
		{
			try
			{
				if (AppSettings.IS_SQL_SERVER_PROVIDER)
					return Conexao.QuerySingleOrDefault<DocumentoEntidade>("SELECT * FROM WEB_DOCUMENTO WHERE OID_DOCUMENTO = @OID_DOCUMENTO", new { OID_DOCUMENTO }, Transaction);
		...
		public virtual List<DocumentoEntidade> BuscarPorPasta(decimal? OID_DOCUMENTO_PASTA)
		{
			...
			{
				if (AppSettings.IS_SQL_SERVER_PROVIDER)
					return Conexao.Query<DocumentoEntidade>("SELECT * FROM WEB_DOCUMENTO WHERE ((OID_DOCUMENTO_PASTA = @OID_DOCUMENTO_PASTA)    OR (@OID_DOCUMENTO_PASTA IS NULL AND OID_DOCUMENTO_PASTA IS NULL))", new { OID_DOCUMENTO_PASTA }, Transaction).ToList();
				else if (AppSettings.IS_ORACLE_PROVIDER)
					return Conexao.Query<DocumentoEntidade>("SELECT * FROM WEB_DOCUMENTO WHERE ((OID_DOCUMENTO_PASTA=:OID_DOCUMENTO_PASTA) OR (:OID_DOCUMENTO_PASTA IS NULL  AND OID_DOCUMENTO_PASTA IS NULL ))", new { OID_DOCUMENTO_PASTA }, Transaction).ToList();
				else
					throw new Exception("Provider não suportado!");
		...
		public virtual List<DocumentoEntidade> BuscarPorPastaPlano(decimal? OID_DOCUMENTO_PASTA, string CD_PLANO, string ORDER_CRITERIA)
		{
			...
			{
				if (AppSettings.IS_SQL_SERVER_PROVIDER)
					return Conexao.Query<DocumentoEntidade>("SELECT * FROM WEB_DOCUMENTO D  LEFT JOIN WEB_DOCUMENTO_PLANO DP ON D.OID_DOCUMENTO = DP.OID_DOCUMENTO LEFT JOIN TB_PLANOS P ON DP.CD_PLANO = P.CD_PLANO WHERE (OID_DOCUMENTO_PASTA = @OID_DOCUMENTO_PASTA OR @OID_DOCUMENTO_PASTA IS NULL)   AND (P.CD_PLANO = @CD_PLANO OR @CD_PLANO IS NULL) ORDER BY      CASE        WHEN @ORDER_CRITERIA = 'nome' THEN D.TXT_TITULO     END,     CASE 	    WHEN @ORDER_CRITERIA = 'data' THEN D.DTA_INCLUSAO     END", new { OID_DOCUMENTO_PASTA, CD_PLANO, ORDER_CRITERIA }, Transaction).ToList();
				else if (AppSettings.IS_ORACLE_PROVIDER)
					return Conexao.Query<DocumentoEntidade>("SELECT * FROM WEB_DOCUMENTO  D  LEFT JOIN WEB_DOCUMENTO_PLANO   DP  ON D.OID_DOCUMENTO=DP.OID_DOCUMENTO LEFT JOIN TB_PLANOS   P  ON DP.CD_PLANO=P.CD_PLANO WHERE (OID_DOCUMENTO_PASTA=:OID_DOCUMENTO_PASTA OR :OID_DOCUMENTO_PASTA IS NULL ) AND (P.CD_PLANO=:CD_PLANO OR :CD_PLANO IS NULL ) ORDER BY CASE  WHEN :ORDER_CRITERIA='NOME' THEN D.TXT_TITULO END , CASE  WHEN :ORDER_CRITERIA='DATA' THEN D.DTA_INCLUSAO END ", new { OID_DOCUMENTO_PASTA, CD_PLANO, ORDER_CRITERIA }, Transaction).ToList();
				else
					throw new Exception("Provider não suportado!");
		...
		public virtual List<DocumentoEntidade> BuscarPorPastaPlanoOrderByTxtTitulo(decimal? OID_DOCUMENTO_PASTA, string CD_PLANO, string ORDER_CRITERIA)
		{
			try
			{
				if (AppSettings.IS_SQL_SERVER_PROVIDER)
					return Conexao.Query<DocumentoEntidade>("SELECT *  FROM WEB_DOCUMENTO D LEFT JOIN WEB_DOCUMENTO_PLANO DP  ON D.OID_DOCUMENTO = DP.OID_DOCUMENTO  LEFT JOIN TB_PLANOS P  ON DP.CD_PLANO = P.CD_PLANO  WHERE ((OID_DOCUMENTO_PASTA = @OID_DOCUMENTO_PASTA)    OR (@OID_DOCUMENTO_PASTA IS NULL AND OID_DOCUMENTO_PASTA IS NULL))    AND ((P.CD_PLANO = @CD_PLANO  OR DP.CD_PLANO IS NULL)    OR (@CD_PLANO IS NULL AND P.CD_PLANO IS NULL))    ORDER BY D.TXT_TITULO", new { OID_DOCUMENTO_PASTA, CD_PLANO, ORDER_CRITERIA }, Transaction).ToList();
				else if (AppSettings.IS_ORACLE_PROVIDER)
					return Conexao.Query<DocumentoEntidade>("SELECT * FROM WEB_DOCUMENTO  D  LEFT JOIN WEB_DOCUMENTO_PLANO   DP  ON D.OID_DOCUMENTO=DP.OID_DOCUMENTO LEFT JOIN TB_PLANOS   P  ON DP.CD_PLANO=P.CD_PLANO WHERE ((OID_DOCUMENTO_PASTA=:OID_DOCUMENTO_PASTA) OR (:OID_DOCUMENTO_PASTA IS NULL  AND OID_DOCUMENTO_PASTA IS NULL )) AND ((P.CD_PLANO=:CD_PLANO OR DP.CD_PLANO IS NULL ) OR (:CD_PLANO IS NULL  AND P.CD_PLANO IS NULL )) ORDER BY D.TXT_TITULO", new { OID_DOCUMENTO_PASTA, CD_PLANO, ORDER_CRITERIA }, Transaction).ToList();
				else
					throw new Exception("Provider não suportado!");
		...
		public virtual List<DocumentoEntidade> BuscarPorPlanoPasta(string CD_PLANO, decimal? OID_DOCUMENTO_PASTA)
		{
			...
			{
				if (AppSettings.IS_SQL_SERVER_PROVIDER)
					return Conexao.Query<DocumentoEntidade>("SELECT WEB_DOCUMENTO.* FROM WEB_DOCUMENTO WHERE OID_DOCUMENTO NOT IN (SELECT OID_DOCUMENTO FROM  WEB_DOCUMENTO_PLANO WHERE CD_PLANO <> @CD_PLANO)   AND ((OID_DOCUMENTO_PASTA = @OID_DOCUMENTO_PASTA)    OR (@OID_DOCUMENTO_PASTA IS NULL AND OID_DOCUMENTO_PASTA IS NULL))", new { CD_PLANO, OID_DOCUMENTO_PASTA }, Transaction).ToList();
				else if (AppSettings.IS_ORACLE_PROVIDER)
					return Conexao.Query<DocumentoEntidade>("SELECT WEB_DOCUMENTO.* FROM WEB_DOCUMENTO WHERE OID_DOCUMENTO NOT  IN (SELECT OID_DOCUMENTO FROM WEB_DOCUMENTO_PLANO WHERE CD_PLANO<>:CD_PLANO) AND ((OID_DOCUMENTO_PASTA=:OID_DOCUMENTO_PASTA) OR (:OID_DOCUMENTO_PASTA IS NULL  AND OID_DOCUMENTO_PASTA IS NULL ))", new { CD_PLANO, OID_DOCUMENTO_PASTA }, Transaction).ToList();
				else
					throw new Exception("Provider não suportado!");
