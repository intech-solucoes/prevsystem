...
namespace Intech.PrevSystem.Dados.DAO
{
	public abstract class FundacaoDAO : BaseDAO<FundacaoEntidade>
	{
		...
		public virtual List<FundacaoEntidade> BuscarTodas()
		{
			try
			{
				if (AppSettings.IS_SQL_SERVER_PROVIDER)
					return Conexao.Query<FundacaoEntidade>("SELECT   	ENT.NOME_ENTID,  	TB_FUNDACAO.*  FROM TB_FUNDACAO  INNER JOIN EE_ENTIDADE ENT ON ENT.COD_ENTID = TB_FUNDACAO.COD_ENTID", new {  }).ToList();
				else if (AppSettings.IS_ORACLE_PROVIDER)
					return Conexao.Query<FundacaoEntidade>("SELECT ENT.NOME_ENTID, TB_FUNDACAO.* FROM TB_FUNDACAO INNER  JOIN EE_ENTIDADE   ENT  ON ENT.COD_ENTID=TB_FUNDACAO.COD_ENTID", new {  }).ToList();
				else
					throw new Exception("Provider não suportado!");
