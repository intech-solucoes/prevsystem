...
namespace Intech.PrevSystem.Dados.DAO
{
	public abstract class RecebedorBeneficioDAO : BaseDAO<RecebedorBeneficioEntidade>
	{
		...
		public virtual RecebedorBeneficioEntidade BuscarPorFundacaoEmpresaInscricaoPlano(string CD_FUNDACAO, string CD_EMPRESA, string NUM_INSCRICAO, string CD_PLANO)
		{
			try
			{
				if (AppSettings.IS_SQL_SERVER_PROVIDER)
					return Conexao.QuerySingleOrDefault<RecebedorBeneficioEntidade>("SELECT RB.*  FROM GB_RECEBEDOR_BENEFICIO RB      INNER JOIN GB_PROCESSOS_BENEFICIO PB ON PB.CD_FUNDACAO = RB.CD_FUNDACAO          AND PB.NUM_INSCRICAO = RB.NUM_INSCRICAO      INNER JOIN GB_ESPECIE_BENEFICIO EB ON EB.CD_ESPECIE = PB.CD_ESPECIE  WHERE PB.DT_TERMINO >= GETDATE()    AND RB.CD_FUNDACAO = @CD_FUNDACAO    AND RB.CD_EMPRESA = @CD_EMPRESA    AND RB.NUM_INSCRICAO = @NUM_INSCRICAO    AND PB.CD_PLANO = @CD_PLANO", new { CD_FUNDACAO, CD_EMPRESA, NUM_INSCRICAO, CD_PLANO });
				else if (AppSettings.IS_ORACLE_PROVIDER)
					return Conexao.QuerySingleOrDefault<RecebedorBeneficioEntidade>("SELECT RB.* FROM GB_RECEBEDOR_BENEFICIO  RB  INNER  JOIN GB_PROCESSOS_BENEFICIO   PB  ON PB.CD_FUNDACAO=RB.CD_FUNDACAO AND PB.NUM_INSCRICAO=RB.NUM_INSCRICAO INNER  JOIN GB_ESPECIE_BENEFICIO   EB  ON EB.CD_ESPECIE=PB.CD_ESPECIE WHERE PB.DT_TERMINO>=SYSDATE AND RB.CD_FUNDACAO=:CD_FUNDACAO AND RB.CD_EMPRESA=:CD_EMPRESA AND RB.NUM_INSCRICAO=:NUM_INSCRICAO AND PB.CD_PLANO=:CD_PLANO", new { CD_FUNDACAO, CD_EMPRESA, NUM_INSCRICAO, CD_PLANO });
				else
					throw new Exception("Provider não suportado!");
			}
			finally
