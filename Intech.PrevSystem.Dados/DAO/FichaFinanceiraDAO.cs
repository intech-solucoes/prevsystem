...
namespace Intech.PrevSystem.Dados.DAO
{
	public abstract class FichaFinanceiraDAO : BaseDAO<FichaFinanceiraEntidade>
	{
		...
		public virtual List<FichaFinanceiraEntidade> Sabesprev_BuscarInformePlanosPorFundacaoInscricaoAno(string CD_FUNDACAO, string NUM_INSCRICAO, string ANO)
		{
			try
			{
				if (AppSettings.IS_SQL_SERVER_PROVIDER)
					return Conexao.Query<FichaFinanceiraEntidade>("", new { CD_FUNDACAO, NUM_INSCRICAO, ANO }).ToList();
				else if (AppSettings.IS_ORACLE_PROVIDER)
					return Conexao.Query<FichaFinanceiraEntidade>("SELECT FF.CD_PLANO,  	   SUM(FF.CONTRIB_PARTICIPANTE) AS CONTRIB_PARTICIPANTE,  	   SUM(FF.CONTRIB_EMPRESA) AS CONTRIB_EMPRESA  FROM CC_FICHA_FINANCEIRA FF  	INNER JOIN CS_FUNCIONARIO FN ON FN.CD_FUNDACAO = FF.CD_FUNDACAO   			AND FN.NUM_INSCRICAO = FF.NUM_INSCRICAO  	INNER JOIN TB_TIPO_CONTRIBUICAO TC ON TC.CD_TIPO_CONTRIBUICAO = FF.CD_TIPO_CONTRIBUICAO  WHERE TC.CK_COMPOE_IR_AM = 'S'    AND FF.CD_FUNDACAO =:CD_FUNDACAO    AND FF.NUM_INSCRICAO =:NUM_INSCRICAO    AND FF.ANO_REF =:ANO    AND FF.CD_ORIGEM NOT IN ('GB', 'G0', 'DR')    AND FF.CD_TIPO_CONTRIBUICAO NOT IN ('73', '70', '74', '34', 'B4', 'B6', 'B5', 'B1', 'B2', 'B3', 'B7', 'B8', 'B9', 'C1', 'C5', 'C6', 'D5')  GROUP BY FF.CD_PLANO", new { CD_FUNDACAO, NUM_INSCRICAO, ANO }).ToList();
				else
					throw new Exception("Provider não suportado!");
			}
			finally
